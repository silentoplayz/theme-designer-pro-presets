...
class Event:
    class Valves(BaseModel):
        ...
        auto_sync: bool = Field(
            default=False,
            description="Skip the Selective Sync modal when loading presets or syncing. When enabled, a 'Sync All' toggle appears in the designer footer — turn it on to propagate changes to all modes, or leave it off to apply only to the active mode.\n\n---\n\n#### 🎨 Visual Style",
        )
        # ── Visual Style ──
        sidebar_transparency: Literal["opaque", "translucent", "transparent"] = Field(
            default="opaque",
            description=(
                "Controls sidebar background style. "
                ...
                "(automatically uses translucent on mobile for readability)."
            ),
        )
        overlay_transparency: Literal["opaque", "translucent"] = Field(
            default="opaque",
            description=(
                "Controls overlay UI (settings modal, dropdowns, integration menus) background style. "
                "'opaque' (default) — solid background for best readability. "
                "'translucent' — frosted glass effect (backdrop-blur) that lets backgrounds show through."
                "\n\n---\n\n#### 🔒 Security"
            ),
        ...
        enable_url_import: bool = Field(
            default=True,
            description="Show the 'Load from URL' import controls in the designer. This is a UI convenience guard on the admin-only designer page (the fetch runs in the admin's own browser), not a server-enforced security boundary.",
        )
        allowed_import_domains: str = Field(
            default="",
            description="Comma-separated domains the designer's URL-import UI will accept (e.g. 'raw.githubusercontent.com, openwebui.com'). Empty = allow any. Enforced client-side on the admin-only page as a guardrail against typos/mistakes — not a security control.\n\n---\n\n#### 🛠️ Advanced",
        )
        # ── Advanced ──
    ...
    (function() {
        ...
        try {
            // Theme active state — controlled by function toggle.
            // When the function is disabled, the bootloader is re-injected with
            // __THEME_ACTIVE__=false to keep SSE alive while stripping the theme.
            // When active, this is always true.
            var _themeActive = __THEME_ACTIVE__;
            if (!_themeActive) {
                try {
                    localStorage.removeItem('owui_dev_theme_v1_css');
                    localStorage.removeItem('owui_dev_theme_v1');
            ...
            // Cached parsed state — invalidated when _owuiThemeState changes.
            // Avoids JSON.parse on every enforceTheme() + initCanvas() call.
            var _parsedState = null;
            // Monotonic version counter — prevents stale fetch responses from overwriting fresher SSE data
            var _themeVersion = 0;
            // Delivery ETags of the last APPLIED css/state — when an SSE broadcast
            // carries matching tags, this client already has that exact version
            // and skips the refetch (avoids a fleet-wide request storm on save).
            var _lastCssTag = null;
            var _lastStateTag = null;

            function getParsedState() {
                if (_parsedState) return _parsedState;
                try {
                    var raw = _owuiThemeState || localStorage.getItem('owui_dev_theme_v1') || '{}';
                    if (typeof raw === 'string' && raw.charAt(0) === '{') {
                        _parsedState = JSON.parse(raw);
                    }
                } catch(e) { _parsedState = {}; }
                return _parsedState || {};
            }

            // Fetch the latest CSS + state from the server and apply them.
            // Shared by: initial page load, SSE theme-update (the broadcast
            // carries only a version token — clients refetch), and iOS PWA
            // foreground resume. cache: 'no-cache' forces conditional
            // revalidation (If-None-Match) — the server answers 304 when
            ...
            function enforceTheme() {
                ...
                if (savedCss) {
                    ...
                    // Palette vars for disabled modes are emitted as defaults
                    // by generateCoreCSS(), so no bootloader stripping needed.
                    var parsedState = getParsedState();

                    if (parsedState) {
                        if (isAuthPage) {
                            try {
                                const mode = themeToMode(localStorage.getItem('theme') || 'dark');
                                const config = parsedState[mode];
                            
                                if (config) {
                                    var sec = parsedState._cssSections;
                                    if (sec && typeof sec.vars === 'string') {
                                        // Structured sections (merged into state.json by the
                                        // server at delivery time): assemble only the sections
                                        // allowed on auth pages — no regex-slicing needed.
                                        var parts = [];
                                        if (config.themeShowAuth !== false) parts.push(sec.vars);
                                        if (!(config.canvasShowAuth === false && config.gradientShowAuth === false)) parts.push(sec.structural || '');
                                        if (config.gradientShowAuth !== false) parts.push(sec.gradient || '');
                                        if (config.customCssShowAuth !== false) parts.push(sec.custom || '');
                                        parts.push(sec.extra || ''); // valve-appended rules — never auth-gated
                                        // Re-apply the body::before sanitizer (assembly replaced finalCss)
                                        finalCss = parts.join('').replace(/[^{}]*body\s*::before\s*\{[^}]*\}/g, '');
                                    } else {
                                        // Legacy fallback: saved theme predates structured
                                        // sections — regex-strip the flat CSS by markers.
                ...
                if (storedTheme) {
                    ...
                    if (target) {
                        if (!html.classList.contains(target)) html.classList.add(target);
                        ['dark', 'light', 'her'].filter(c => c !== target).forEach(c => html.classList.remove(c));
                    }
                }
            }
            
            // Token usage interceptor — listens for Socket.IO 'events' messages
            // containing usage data from chat:completion responses.
            // Open WebUI uses WebSocket (Socket.IO) for chat, not HTTP fetch.
            ...
            (function() {
                ...
                // Intercept WebSocket connections to extract token usage data
                // from Socket.IO chat:completion events for context-aware Canvas FX.
                // Uses a constructor wrapper instead of prototype patching so that
                // each new WebSocket instance gets a listener at creation time
                // without modifying WebSocket.prototype.
                function hookSocket() {
                    function attachUsageListener(ws) {
                        console.debug('[Theme Pro] WebSocket hooked for usage interception');
                        ws.addEventListener('message', function(evt) {
                            try {
                                // Socket.IO v4 protocol: '42["event_name",{data}]'
                                var raw = typeof evt.data === 'string' ? evt.data : '';
                                if (raw.indexOf('"usage"') === -1) return;
                                // Strip Socket.IO packet prefix (e.g. '42')
                                var jsonStart = raw.indexOf('[');
                                if (jsonStart === -1) return;
                                var arr = JSON.parse(raw.slice(jsonStart));
                                if (!Array.isArray(arr) || arr.length < 2) return;
                                console.debug('[Theme Pro] WS frame with usage detected, event:', arr[0]);
                                var payload = arr[1];
                                // Walk payload recursively for usage data.
                                // Socket.IO event nesting:
                                //   arr[1].data.data.usage  (chat:completion)
                                //   arr[1].data.usage       (direct)
                                //   arr[1].usage            (flat)
                                function findUsage(obj, depth) {
                                    if (!obj || typeof obj !== 'object' || depth > 4) return null;
                                    if (obj.usage && typeof obj.usage === 'object' &&
                                        (obj.usage.total_tokens || obj.usage.prompt_tokens ||
                                         obj.usage.completion_tokens || obj.usage.input_tokens ||
                                         obj.usage.output_tokens)) {
                                        return obj.usage;
                                    }
                                    if (obj.data) return findUsage(obj.data, depth + 1);
                                    return null;
                                }
                                var usage = findUsage(payload, 0);
                                if (usage && (usage.total_tokens || usage.input_tokens || usage.output_tokens || usage.prompt_tokens || usage.completion_tokens)) {
                                    var total = usage.total_tokens || ((usage.input_tokens || usage.prompt_tokens || 0) + (usage.output_tokens || usage.completion_tokens || 0));
                                    ...
                                    console.debug('[Theme Pro] Dispatched exactTokens:', total, 'prompt:', usage.prompt_tokens || usage.input_tokens, 'completion:', usage.completion_tokens || usage.output_tokens);
                                }
                            } catch(x) { console.debug('[Theme Pro] WS intercept error:', x); }
                        });
                    }

                    // Wrap the WebSocket constructor to hook each new instance
                    var _NativeWebSocket = window.WebSocket;
                    window.WebSocket = function(url, protocols) {
                        var ws = protocols !== undefined
                            ? new _NativeWebSocket(url, protocols)
                            : new _NativeWebSocket(url);
                        attachUsageListener(ws);
                        return ws;
                    };
                    // Preserve prototype chain and static constants
                    window.WebSocket.prototype = _NativeWebSocket.prototype;
                    window.WebSocket.CONNECTING = _NativeWebSocket.CONNECTING;
                    window.WebSocket.OPEN = _NativeWebSocket.OPEN;
                    window.WebSocket.CLOSING = _NativeWebSocket.CLOSING;
                    window.WebSocket.CLOSED = _NativeWebSocket.CLOSED;
                }
                hookSocket();
            })();
                    ...
                    if (!_navObserver) {
                        _navObserver = new MutationObserver(function() {
                            if (!document.getElementById('messages-container')) {
                                ...
                                // Send zero-state only once (when transitioning away from chat)
                                if (_lastChars !== 0 || _lastMsgCount !== 0) {
                                    _lastChars = 0;
                                    _lastMsgCount = 0;
            ...
            function initCanvas(forceFallback) {
                ...
                try {
                    ...
                    if (config && config.canvasEnabled && config.canvasScript) {
                        ...
                        // Pre-detect scripts that reference `document` — they'll always
                        // throw ReferenceError in a Worker. Skip straight to fallback
                        // to avoid async onerror race with rapid initCanvas() calls.
                        const _scriptNeedsDOM = config.canvasScript.indexOf('document') !== -1;
                        const useWorker = !forceFallback && !_scriptNeedsDOM && !!canvas.transferControlToOffscreen;
                        ...
                        if (useWorker) {
                            ...
                            const _env = {
                                authToken: __CANVAS_API_ACCESS__ ? ((document.cookie.match(/token=([^;]+)/) || [])[1] || localStorage.getItem('token') || '') : '',
                            ...
                            try {
                                (function() {
                                    ...
                                    // Declare _fbHandler early so all event dispatchers can reference it.
                                    // The canvas script sets self.onmessage which writes to _fbHandler (L714).
                                    // Also exposed via window so the bootloader's _canvasSend can reach it.
                                    let _fbHandler = null;
                                    Object.defineProperty(window, '__owuiFbHandler', {
                                        get: function() { return _fbHandler; },
                                        set: function(fn) { _fbHandler = fn; },
                                        configurable: true
                                    });
                                    ...
                                    const _onResize = () => {
                                        ...
                                        _resizeTimer = setTimeout(() => {
                                            ...
                                            if (_fbHandler) _fbHandler({ data: { type: 'resize', width: window.innerWidth, height: window.innerHeight } });
                                        }, 150);
                                    };
                                    // Bridge: receive forwarded mousemove from child iframes (they can't bubble to parent window)
                                    const _onIframeMsg = (e) => { if (e.data && e.data.type === 'owui-canvas-mousemove') { _mouseX = e.data.x; _mouseY = e.data.y; _mouseDirty = true; } };
                                    ...
                                    // Click + pointer events — discrete, sent immediately
                                    const _fbSend = (type, data) => { if (_fbHandler) _fbHandler({ data: Object.assign({ type: type }, data) }); };
                            ...
                            _canvasSend = function(data) { if (window.__owuiFbHandler) window.__owuiFbHandler({ data: data }); };
                        }
            ...
            window.addEventListener('owui-theme-updated', function(e) {
                ...
                if (e.detail && e.detail.state) { _owuiThemeState = e.detail.state; _parsedState = null; }
                if (e.detail && e.detail.css) _owuiThemeCss = e.detail.css;
                refresh();
            });
            window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', refresh);

            // SPA navigation is handled by the popstate listener (L874) and
            // MutationObserver (L847-862) which fire on every SvelteKit route
            // change. No history.pushState/replaceState patches needed.

            // Live theme push via Server-Sent Events (cross-tab/browser/device)
            if (!window.__THEME_DESIGNER__) {
                try {
                    // --- SSE handler functions (shared between initial and reconnected EventSource) ---
                    function handleThemeUpdate(e) {
                        _disabled = false;  // Re-enable if admin turned it back on
                        // The broadcast carries only a version token (+ delivery
                        // ETags) — clients refetch the post-processed CSS + state.
                        // This keeps SSE frames tiny (previously every save pushed
                        // the full CSS + state, including the entire canvas script,
                        // to every client).
                        try {
                            var d = JSON.parse(e.data || '{}');
                            if (d.cssTag && d.stateTag &&
                                d.cssTag === _lastCssTag && d.stateTag === _lastStateTag) {
                                return; // Already applied this exact version — skip refetch
                            }
                        } catch(x) {}
                        var myV = ++_themeVersion;  // Supersede any in-flight fetch
                        // Jitter the refetch 0-2s so one broadcast doesn't stampede
                        // the server with simultaneous fetches from every client.
                        setTimeout(function() {
                            if (myV !== _themeVersion || _disabled) return; // Superseded while waiting
                            fetchThemeFromServer();
                        }, Math.random() * 2000);
                    }
                    function handleThemeDisable() {
                        // Admin disabled or reset — set disable flag, clear state, strip DOM
                        _disabled = true;
                        _themeVersion++;  // Invalidate any in-flight fetch callbacks
                        _owuiThemeState = null;
                        _owuiThemeCss = null;
                        _parsedState = null;  // Invalidate cached parse
                        _lastCssTag = null;   // Theme is no longer applied — clear the
                        _lastStateTag = null; // ETags so a re-enable with an unchanged theme
                                              // still refetches instead of skipping as "already applied"
                    ...
                    function connectSSE() {
                        ...
                        es = new EventSource('__THEME_ROUTE__/events?_=' + Date.now());
                        es.addEventListener('theme-update', handleThemeUpdate);
                        es.addEventListener('theme-disable', handleThemeDisable);
                    ...
                    // Uses `es` directly (mutable var) so beforeunload always closes the
                    // CURRENT EventSource, even after iOS PWA reconnect reassigns it.
                    window.addEventListener('beforeunload', function() { if (es) es.close(); });

                    // iOS PWA resilience: when the app returns to foreground, the EventSource
                    // connection may have been silently killed by WebKit. Force a reconnect
...
"""
    ...
    def _get_route_base(self) -> str:
        ...
        # Sanitize: only allow alphanumeric, hyphens, underscores, slashes, and dots
        route = _re.sub(r'[^a-zA-Z0-9/_.-]', '', route)
    ...
    @staticmethod
    def _strip_all_theme_blocks(content: str) -> str:
        """Strip all server theme CSS and state JSON blocks from HTML content."""
        markers = [
            ('<style id="owui-server-theme">', '</style><!-- /owui-server-theme -->'),
            ('<script type="application/json" id="owui-theme-state">', '</script><!-- /owui-theme-state -->'),
        ]
    ...
    # Cached result of _find_index_file() — index.html path never changes at runtime
    ...
    def _find_index_file(self):
        ...
        try:
            ...
            if idx.exists():
                Event._cached_index_path = str(idx)
                return Event._cached_index_path
        except ImportError:
            pass
    ...
    def _save_state(self, state: str) -> None:
        """Write theme state JSON to disk atomically."""
        path = self._get_state_path()
        Event._atomic_write(path, state)
    ...
    def _get_sections_path(self) -> Path:
        """Return the path to the persisted CSS sections JSON file."""
        return self._get_css_path().with_name("open_theme_designer_sections.json")

    def _save_sections(self, sections: dict) -> None:
        """Write structured CSS sections to disk atomically."""
        path = self._get_sections_path()
        Event._atomic_write(path, _json.dumps(sections))
        log.info("[Theme Pro] Saved CSS sections to %s", path)

    def _load_sections(self) -> dict | None:
        """Read structured CSS sections from disk, or None if missing/invalid.

        None means "no sections available" — callers fall back to regex-slicing
        the flat CSS (themes saved by pre-sections designer builds).
        """
        path = self._get_sections_path()
        if not path.exists():
            return None
        ...
        if not isinstance(data, dict) or not all(
            isinstance(data.get(k), str) for k in self.SECTION_KEYS
        ):
            return None
        return {k: data[k] for k in self.SECTION_KEYS}

    def _delete_sections(self) -> None:
        """Remove the sections file (stale-prevention when only flat CSS is saved)."""
        path = self._get_sections_path()
        if path.exists():
            path.unlink()
            log.info("[Theme Pro] Deleted CSS sections file")
    ...
    def _save_library(self, library: str) -> None:
        """Write preset/snapshot library JSON to disk atomically."""
        path = self._get_library_path()
        Event._atomic_write(path, library)
    ...
    # -- delivery assembly -----------------------------------------------------
    # Blocking (stat + read + regex post-processing): route handlers call these
    # via asyncio.to_thread so a cache miss never stalls the event loop.

    def _get_css_delivery(self):
        """Return (css, etag) for /theme.css delivery, or None when unavailable.

        Post-processing runs regexes over the full CSS — cache the result per
        (file mtime, valve settings) so the steady-state cost is a stat().
        Every page load of every user hits this path. Read errors (including
        the file vanishing between stat() and read during a reset) resolve to
        None — callers serve the graceful empty response, never a 500.
        """
        try:
        ...
        cached = Event._css_delivery_cache
        ...
        if not raw:
            return None
        css = self._postprocess_css(raw)
        # usedforsecurity=False: this is a cache validator, not a security
        # digest — the flag keeps md5 usable on FIPS hosts (where a bare
        # md5() raises ValueError).
        etag = '"' + hashlib.md5(css.encode("utf-8"), usedforsecurity=False).hexdigest() + '"'
        Event._css_delivery_cache = (cache_key, css, etag)
        return css, etag

    def _get_state_delivery(self):
        """Return (state, etag) for /state.json delivery, or None when unavailable.

        Mirrors _get_css_delivery: cached per (state mtime, sections mtime,
        valve settings), OSError-safe against reset races.
        """
        try:
            state_mtime = self._get_state_path().stat().st_mtime_ns
        except OSError:
            return None
        try:
        ...
        cached = Event._state_delivery_cache
        if cached and cached[0] == cache_key:
            return cached[1], cached[2]
        try:
        ...
        state = self._state_with_sections(state)
        # usedforsecurity=False — see _get_css_delivery (FIPS-safe).
        etag = '"' + hashlib.md5(state.encode("utf-8"), usedforsecurity=False).hexdigest() + '"'
        Event._state_delivery_cache = (cache_key, state, etag)
        return state, etag

    def _prime_delivery_caches(self) -> dict:
        """Recompute both delivery caches; return their ETags for broadcasting.

        Called via asyncio.to_thread right before a theme-update broadcast so
        (a) the etags ride in the SSE payload — clients already holding that
        exact version skip the refetch — and (b) the first post-save fetches
        hit a warm cache instead of all racing the post-processing pipeline.
        """
    ...
    def _register_route(self, app) -> None:
        ...
        # When the module is reloaded, Event._sse_clients resets to empty, but app.state
        # persists. Re-aliasing ensures broadcast methods push to ALL clients (including
        # pre-reload ones). A set gives O(1) disconnect cleanup — with a list, mass
        # disconnects (deploys) degrade to O(n^2) scans.
        _existing_clients = getattr(app.state, "_theme_sse_clients", None)
        if _existing_clients is None:
            app.state._theme_sse_clients = set()
        elif not isinstance(_existing_clients, set):
            # Pre-set builds stored a list — convert, keeping live client queues
            app.state._theme_sse_clients = set(_existing_clients)
        ...
        async def save_theme_css(request: Request):
            ...
            # CSRF guard: auth accepts the session cookie, and a cross-origin
            # page can send a cookie-bearing "simple request" (no preflight) as
            # long as it doesn't set a JSON Content-Type. Requiring
            # application/json forces a CORS preflight on any cross-origin
            # attempt, which fails — the designer always sends this header.
            ...
            # Reject oversized payloads up front — the per-field caps in
            # _handle_css_save only run after request.json() has already
            # buffered the entire body in memory. 64MB comfortably covers
            # the field caps (5+5+20MB) plus JSON escaping overhead.
            try:
                if int(request.headers.get("content-length") or 0) > 64 * 1024 * 1024:
                    return JSONResponse(
                        {"error": "Request body exceeds 64MB limit"},
                        status_code=413,
                    )
            except ValueError:
        ...
        async def serve_theme_css(request: Request):
            ...
            Sends an ETag and honors If-None-Match: the bootloader fetches with
            cache 'no-cache' (conditional revalidation), so unchanged CSS costs
            a 304 instead of a full-body transfer on every page load. Delivery
            ...
            delivery = await asyncio.to_thread(event_instance._get_css_delivery)
            if delivery is None:
                # No saved CSS (fresh install, reset, or read race). Empty 200
                # (not 204 — a 204 must not carry a body, and some proxies
                # rewrite or warn on it). The bootloader treats an empty body
                # as "theme inactive".
                return PlainTextResponse(
                    "", media_type="text/css", headers={"Cache-Control": "no-store"}
                )
            ...
            return PlainTextResponse(
                content=css, media_type="text/css", headers=headers
            )
        ...
        async def serve_theme_state(request: Request):
            ...
            ETag/If-None-Match handling mirrors serve_theme_css — unchanged
            state costs a 304 instead of re-sending the full canvas script.
            ...
            delivery = await asyncio.to_thread(event_instance._get_state_delivery)
            if delivery is None:
                # No saved state (fresh install, reset, or read race).
                # Empty-object 200 (not 204 — see serve_theme_css). The
                # bootloader treats '{}' as "no saved state".
                return PlainTextResponse(
                    "{}",
                    media_type="application/json",
                    headers={"Cache-Control": "no-store"},
            ...
            if request.headers.get("if-none-match") == etag:
                return PlainTextResponse("", status_code=304, headers=headers)
            return PlainTextResponse(
                content=state, media_type="application/json", headers=headers
            )
        ...
        async def theme_sse(request: Request):
            ...
            # Connection cap: this endpoint is unauthenticated (auth pages need
            # live updates too), so bound per-worker resource usage. Over-cap
            # clients get a valid SSE body that only sets a long retry interval
            # and then ends — EventSource retries later, and the bootloader's
            # visibilitychange refetch keeps them themed in the meantime.
            if len(Event._sse_clients) >= Event._SSE_MAX_CLIENTS:
            ...
            if _hb is None or _hb.done():
                ...
                async def _sse_heartbeat():
                    while True:
                        await asyncio.sleep(15)  # 15s for Cloudflare compatibility
                        for q in list(Event._sse_clients):
                            try:
                                q.put_nowait(": heartbeat\n\n")
                            except asyncio.QueueFull:
                                pass  # Client backed up — skip, data msgs matter more

                request.app.state._theme_sse_heartbeat = (
                    asyncio.get_running_loop().create_task(_sse_heartbeat())
                )

            queue = asyncio.Queue(maxsize=8)
            Event._sse_clients.add(queue)
            ...
            async def event_stream():
                try:
                    ...
                    Event._sse_clients.discard(queue)
        ...
        async def serve_theme_library(request):
            ...
            if auth is not True:
                return auth
            try:
                library = await asyncio.to_thread(self._load_library)
            except OSError:
                library = None  # Reset race / unreadable data dir — serve empty
            if library:
    ...
    async def _handle_css_save(self, request) -> "JSONResponse":
        ...
        try:
            ...
            # Size caps — admin-only endpoint, but a runaway payload would be
            # written to disk and trigger a refetch on every connected client.
            for _name, _val, _cap_mb in (
                ("css", css, 5),
                ("state", state, 5),
                ("library", body.get("library", ""), 20),
            ):
                if isinstance(_val, str) and len(_val) > _cap_mb * 1024 * 1024:
                    return JSONResponse(
                        {"error": f"{_name} exceeds {_cap_mb}MB limit"},
                        status_code=413,
                    )
            if isinstance(sections, dict):
                _total = sum(len(v) for v in sections.values() if isinstance(v, str))
                if _total > 5 * 1024 * 1024:
                    return JSONResponse(
                        {"error": "sections exceeds 5MB limit"}, status_code=413
                    )
            ...
            # Persist the structured sections alongside the flat CSS. When the
            # payload has no valid sections (older designer build still cached
            # in a browser tab), drop any stale sections file so consumers fall
            # back to regex-slicing the fresh flat CSS instead of assembling
            # sections from a previous save.
            if isinstance(sections, dict) and all(
                isinstance(sections.get(k), str) for k in self.SECTION_KEYS
            ):
                self._save_sections({k: sections[k] for k in self.SECTION_KEYS})
            else:
                self._delete_sections()

            if state and isinstance(state, str):
                # '_cssSections' is a delivery-time key (merged into state.json
                # when served) — strip it if a client echoed it back, so it
                # never round-trips into the persisted state.
                if '"_cssSections"' in state:
                    try:
                        _parsed = _json.loads(state)
                        if isinstance(_parsed, dict) and _parsed.pop("_cssSections", None) is not None:
                            state = _json.dumps(_parsed, separators=(",", ":"))
                    except Exception:
                        pass
            ...
            resp = {"status": "ok", "bytes": len(css)}
            # Return the valve-postprocessed CSS when it differs from what was
            # submitted, so the designer can write the same CSS every other
            # client receives into its localStorage fallback (raw CSS remains
            # the source of truth on disk). State is deliberately NOT returned:
            # the designer's localStorage state key doubles as its editing
            # store, and writing back a canvas-stripped copy would lose data.
            processed_css = self._postprocess_css(css)
            if processed_css != css:
                resp["css"] = processed_css
            return JSONResponse(resp)
        except Exception as e:
            log.exception("[Theme Pro] Error saving CSS")
            return JSONResponse({"error": "Failed to save theme data"}, status_code=500)
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-font-smoothing: antialiased; }
        ...
        .tag-filter-dot-webgl { background: #a78bfa; }
        .tag-filter-dot-context { background: #2dd4bf; }
        .tag-filter-dot-dom { background: #60a5fa; }
        ...
        .tabs { display: flex; background: var(--bg-deep); padding: 6px; border-radius: 16px; border: 1px solid var(--border); overflow-x: auto; scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.15) transparent; -webkit-overflow-scrolling: touch; flex-shrink: 0; z-index: 90; margin: 12px auto 0; width: calc(100% - 24px); max-width: 1920px; box-sizing: border-box; }
        ...
        .content-area { padding: 20px 12px; display: flex; flex-direction: column; gap: 20px; flex: 1; overflow-y: auto; overflow-x: hidden; max-width: 1920px; margin: 0 auto; width: 100%; box-sizing: border-box; min-height: 0; }
        ...
        /* Update Diff View — reuses sync-diff infrastructure */
        .update-diff-container { margin: 0; max-height: 320px; overflow-y: auto; scrollbar-width: thin; scrollbar-color: var(--accent) transparent; }
        .update-diff-container::-webkit-scrollbar { width: 4px; }
        .update-diff-container::-webkit-scrollbar-thumb { background: var(--accent); border-radius: 4px; }
        .update-diff-section { margin-bottom: 8px; }
        .update-diff-header { display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 8px 10px; background: var(--bg-deep); border: 1px solid var(--border); border-radius: 8px; font-size: 0.65rem; font-weight: 700; color: var(--text-main); transition: 0.2s; user-select: none; }
        .update-diff-header:hover { border-color: var(--accent); }
        .update-diff-header .mode-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
        .update-diff-header .chevron { font-size: 8px; margin-left: auto; opacity: 0.4; transition: transform 0.25s; }
        .update-diff-header.expanded .chevron { transform: rotate(180deg); opacity: 0.8; }
        .update-diff-body { padding: 8px 0 4px 12px; border-left: 2px solid color-mix(in srgb, var(--accent) 40%, transparent); display: none; }
        .update-diff-body.visible { display: block; animation: diffSlideIn 0.25s ease; }
        .update-diff-badge { font-size: 0.5rem; padding: 1px 6px; border-radius: 10px; font-weight: 600; margin-left: auto; }
        .update-diff-badge.changed { background: rgba(234,179,8,0.12); color: #eab308; }
        .update-diff-badge.same { background: rgba(34,197,94,0.12); color: #22c55e; }
        .update-diff-sub { font-size: 0.55rem; color: var(--text-muted); font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin: 8px 0 4px; opacity: 0.6; }
        .update-diff-row-ignore { display: flex; align-items: center; gap: 6px; }
        ...
        .var-lock, .var-reset { cursor: pointer; font-size: 14px; opacity: 0.3; transition: 0.2s; padding: 4px; border-radius: 6px; }
        .var-lock:hover, .var-reset:hover { opacity: 1; background: rgba(255, 255, 255, 0.05); }
        .is-locked .var-lock { opacity: 1; color: var(--accent); }
        .var-reset { display: flex; align-items: center; justify-content: center; }
        ...
        body.light-mode {
            .owui-code-block { background: var(--bg-surface); box-shadow: 0 4px 12px var(--lm-border-subtle), inset 0 0 0 1px var(--lm-shadow); }
            .owui-code-header { background: var(--lm-bg-tint); border-bottom-color: var(--lm-border-subtle); }
            .owui-code-btn:hover { background: var(--lm-border-subtle); }
            .owui-code-block textarea { color: var(--text-main); }
            .owui-line-numbers { color: rgba(0,0,0,0.18); border-right-color: var(--lm-border-subtle); }
        }
        ...
        .doc-accordion summary, .doc-sub-accordion summary { cursor: pointer; display: flex; justify-content: space-between; align-items: center; list-style: none; font-weight: 700; color: var(--text-main); transition: 0.2s; }
        .doc-accordion summary::-webkit-details-marker, .doc-sub-accordion summary::-webkit-details-marker { display: none; }
        .doc-accordion summary:hover, .doc-sub-accordion summary:hover { color: var(--accent); }
        .doc-accordion[open] summary, .doc-sub-accordion[open] summary { color: var(--accent); }
        .doc-accordion summary { padding: 28px 0; font-size: 1.2rem; }
        .doc-accordion[open] summary { padding-bottom: 12px; }
        .doc-accordion summary svg, .doc-sub-accordion summary svg, .doc-sub-accordion summary i[data-icon] { transition: 0.3s; }
        .doc-accordion summary svg { opacity: 0.5; }
        .doc-accordion[open] summary svg, .doc-sub-accordion[open] summary svg, .doc-sub-accordion[open] summary i[data-icon] { transform: rotate(180deg); color: var(--accent); }
        .doc-accordion[open] summary svg { opacity: 1; }
        ...
        .doc-sub-accordion summary { padding: 14px 18px; font-size: 0.82rem; letter-spacing: -0.01em; }
        .doc-sub-accordion summary svg, .doc-sub-accordion summary i[data-icon] { opacity: 0.4; font-size: 0.7rem; }
        .doc-sub-accordion[open] summary svg, .doc-sub-accordion[open] summary i[data-icon] { opacity: 0.8; }
        .doc-sub-accordion[open] summary { border-bottom: 1px solid var(--border); background: rgba(255,255,255,0.02); }
        ...
        .inactive-badge { display: flex; align-items: center; gap: 6px; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.25); color: #ef4444; font-size: 0.7rem; font-weight: 700; letter-spacing: 0.05em; padding: 5px 12px; border-radius: 8px; text-transform: uppercase; white-space: nowrap; transition: background 0.4s, border-color 0.4s, color 0.4s; }
        .inactive-badge.active { background: rgba(34, 197, 94, 0.1); border-color: rgba(34, 197, 94, 0.25); color: #22c55e; }
        .inactive-badge.draft { background: rgba(245, 158, 11, 0.1); border-color: rgba(245, 158, 11, 0.25); color: #f59e0b; }
        .inactive-badge .inactive-dot { width: 7px; height: 7px; border-radius: 50%; background: #ef4444; animation: draft-pulse 1.5s ease-in-out infinite; flex-shrink: 0; transition: background 0.4s; }
        .inactive-badge.active .inactive-dot { background: #22c55e; animation: none; }
        .inactive-badge.draft .inactive-dot { background: #f59e0b; animation: draft-pulse 1.5s ease-in-out infinite; }
        ...
        .docs-modal-close-btn { position: sticky; top: 0; float: right; z-index: 10000; background: var(--bg-deep); border: 1px solid var(--border); color: var(--text-main); font-size: 1.4rem; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: 0.2s; line-height: 1; }
        ...
        .footer { padding: 12px 12px; border-top: none; background: transparent; display: flex; justify-content: space-between; align-items: center; gap: 16px; flex-wrap: wrap; flex-shrink: 0; z-index: 100; max-width: 1920px; margin: 0 auto; width: 100%; box-sizing: border-box; position: relative; }
        .footer-left { display: flex; gap: 12px; align-items: center; flex-wrap: wrap; }
        .sync-all-toggle { display: flex; align-items: center; gap: 5px; cursor: pointer; padding: 4px 10px; border-radius: 12px; border: 1px solid var(--border); background: var(--bg-deep); transition: 0.2s; user-select: none; }
        .sync-all-toggle:hover { border-color: var(--accent); }
        .sync-all-toggle:has(.cb-input:checked) { border-color: var(--accent); background: rgba(16, 185, 129, 0.1); }
        ...
        /* Tablet / narrow windows: icon-only header buttons so mode tabs aren't squished */
        @media (max-width: 768px) {
        ...
        .modal-overlay.z-top { z-index:10001; }
        ...
        .modal-panel, .modal-panel-save { max-width: 95vw; background:var(--bg-surface); border-radius: var(--radius-md); }
        .modal-panel { width: 420px; text-align:center; box-shadow: 0 30px 60px rgba(0,0,0,0.8); padding: 36px; }
        ...
        .modal-panel-save { width: 620px; border: 1px solid var(--accent); box-shadow: 0 20px 50px rgba(0,0,0,1); padding: 32px; }
        ...
        .toggle-label, .toggle-label-gap5, .toggle-label-ml { display: flex; align-items: center; gap: 7px; font-size: 0.65rem; color: var(--text-main); cursor: pointer; }
        .toggle-label-gap5 { gap: 5px; }
        .toggle-label-ml { margin-left: 8px; }
        ...
        .search-clear-btn { position: absolute; right: 4px; width: 18px; height: 18px; border: none; background: rgba(255,255,255,0.08); color: var(--text-muted); font-size: 12px; line-height: 1; cursor: pointer; border-radius: 50%; display: none; align-items: center; justify-content: center; padding: 0; transition: background 0.2s, color 0.2s, opacity 0.3s; opacity: 0; }
        .search-clear-btn:hover { background: rgba(255,255,255,0.15); color: var(--text-main); }
        .search-clear-btn.visible { display: flex; opacity: 1; }
        ...
        .doc-pre, .doc-pre-sm { background: var(--bg-deep); overflow-x: auto; border: 1px solid var(--border); }
        .doc-pre { font-size: 0.65rem; padding: 10px 12px; border-radius: 8px; line-height: 1.6; margin: 8px 0; }
        .doc-pre-sm { font-size: 0.62rem; padding: 8px 10px; border-radius: 7px; line-height: 1.5; margin-bottom: 10px; }
        .sub-label, .sub-label-mb { font-size: 0.65rem; color: var(--text-muted); font-weight: 800; text-transform: uppercase; letter-spacing: 2px; }
        .sub-label-mb { margin-bottom: 10px; }
        .empty-state, .empty-state-lg { text-align: center; color: var(--text-muted); font-size: 0.75rem; border: 1px dashed var(--border); opacity: 0.6; }
        .empty-state { width: 100%; padding: 20px; border-radius: 12px; }
        .empty-state-lg { padding: 40px 20px; border-radius: 20px; }
...
<div class="container" id="theme-designer-container">
    <div class="header">
        <h1>
        ...
        <span class="inactive-badge" id="inactive-badge" data-tooltip="The function is disabled — toggle it ON in Admin → Functions to push themes to users"><span class="inactive-dot"></span> <span id="inactive-badge-text">Theme Inactive</span></span>
    ...
    <div class="content-area">
        ...
        <div id="tab-canvas" class="tab-content" role="tabpanel" style="display:none; height: 100%;">
            <div class="code-container">
                <div class="section-title section-title-bar">
                    <div>Canvas FX <span id="canvas-worker-badge" style="display:none; font-size: 0.55rem; font-weight: 800; padding: 2px 6px; border-radius: 12px; margin-left: 8px; vertical-align: middle; text-transform: uppercase; letter-spacing: 0.05em;"></span><span id="canvas-webgl-badge" style="display:none; font-size: 0.55rem; font-weight: 800; padding: 2px 6px; border-radius: 12px; margin-left: 6px; vertical-align: middle; text-transform: uppercase; letter-spacing: 0.05em; color:#a78bfa; background-color: rgba(167, 139, 250, 0.1);">WebGL</span><span id="canvas-context-badge" style="display:none; font-size: 0.55rem; font-weight: 800; padding: 2px 6px; border-radius: 12px; margin-left: 6px; vertical-align: middle; text-transform: uppercase; letter-spacing: 0.05em; color:#2dd4bf; background-color: rgba(45, 212, 191, 0.1);">Context</span></div>
                    <div class="flex-center">
        ...
        <div id="tab-themes" class="tab-content" role="tabpanel">
            ...
            <div class="section-title" style="margin-top: 28px;">
                ...
                <div class="flex-center-gap6">
                    <div class="search-wrap"><i class="search-icon-overlay"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg></i><input type="text" id="community-search-input" placeholder="Search community themes..." class="search-input-expand" /><button type="button" class="search-clear-btn" id="community-search-input-clear" aria-label="Clear search">&times;</button></div>
                    <button class="btn btn-icon" id="community-search-toggle-btn" data-tooltip="Search Community Themes"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg></button>
                    <button class="btn btn-icon sort-btn" id="community-sort-btn" data-sort="default" data-tooltip="Default Sort"><i class="sort-icon sort-icon-default"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 15l5 5 5-5"/><path d="M7 9l5-5 5 5"/></svg></i><i class="sort-icon sort-icon-asc"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 15l-6-6-6 6"/></svg></i><i class="sort-icon sort-icon-desc"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg></i></button>
            ...
            <div style="max-height: 35vh; overflow-y: auto; overflow-x: hidden; padding: 12px 8px; margin: -12px -8px 16px -8px; width: calc(100% + 16px);">
        ...
        <div id="tab-docs" style="display:none;">
            ...
            <div class="docs-container">
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        ...
                        <ul>
                            ...
                            <li><b>Auto-Save:</b> Your active theme configuration is saved automatically after every change. No manual save is required — use the <b>Save (+)</b> button only to create named snapshots in your theme gallery.</li>
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        <p>Write raw CSS overrides using a robust code editor with auto-indentation and auto-closing pairs (brackets, parentheses, and quotes). Your CSS is saved <i>per-mode</i>. Use the <b>Preset Gallery</b> to save, rename, and manage reusable snippets.</p>
                        <ul>
                        ...
                        <p>You can change the global font family for your entire Open WebUI instance by pasting a CSS rule into the editor. Set the font on the document root and the form controls, then let it <b>inherit</b> down &mdash; form elements (<code>input</code>, <code>textarea</code>, <code>button</code>, <code>select</code>) are listed explicitly because they don't inherit <code>font-family</code> by default. Avoid painting the font onto the universal selector <code>*</code>: it clobbers icon fonts and code/math spans. Here are a few quick examples:</p>
                        ...
                        <pre class="doc-pre-sm" id="font-example-native"><code>html, body, input, textarea, button, select {
  ...
  letter-spacing: -0.01em !important;
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre-sm" id="font-example-mono"><code>html, body, input, textarea, button, select {
  font-family: 'JetBrains Mono', 'Fira Code', 'Consolas', 'Courier New', monospace !important;
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre-sm" id="font-example-serif"><code>html, body, input, textarea, button, select {
  ...
  line-height: 1.65 !important;
                        ...
                        <details class="doc-sub-accordion">
                            <summary>Writing Robust Style Overrides <i data-icon="chevron"></i></summary>
                            <div class="doc-sub-inner">
                                <p>A one-liner like <code>* { border-radius: 2.5px !important; font-family: monospace !important; }</code> <i>works</i>, but the universal selector plus <code>!important</code> is a blunt hammer. It's fine for a throwaway experiment and the wrong foundation for a reusable preset. Four rules of thumb keep overrides robust:</p>
                                <ul>
                                    <li><b>Scope to <code>.app</code>, not <code>*</code>.</b> Open WebUI's whole visible UI lives inside <code>.app</code> (the designer page uses <code>#theme-designer-container</code>). Dropdowns, submenus, modals, and tooltips are <b>portaled onto <code>document.body</code>, outside <code>.app</code></b>. Scoping to <code>.app</code> gives you specificity <code>(0,1,0)</code> for free &mdash; enough to beat a bare Tailwind utility &mdash; while leaving those portaled menus with their native, readable styling.</li>
                                    <li><b>Let fonts inherit; don't paint them on <code>*</code>.</b> Set <code>font-family</code> on <code>html, body</code> (plus the form controls, which don't inherit it) and it cascades everywhere. Putting it on <code>*</code> also overrides icon fonts and code/math spans.</li>
                                    <li><b>Target the utility, not every node.</b> To round corners, hit the elements that already carry a radius utility and skip the fully-round ones &mdash; otherwise you flatten circular avatars, pill toggles, and status dots into rounded squares.</li>
                                    <li><b>Reserve <code>!important</code> for when a utility genuinely out-specifies you.</b> Your Style Overrides snippet is injected <b>last</b> in the cascade (after the palette, structural, and gradient layers), so against the designer's own CSS you rarely need it at all.</li>
                                </ul>
                                <p style="font-size:0.7rem; font-weight:700; margin-top:10px; margin-bottom:4px; opacity:0.9;">Blunt (breaks avatars &amp; toggles)</p>
                                <div style="position: relative;">
                                <pre class="doc-pre-sm" id="so-radius-blunt"><code>* {
  border-radius: 2.5px !important;
}</code></pre>
                                    <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('so-radius-blunt').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                                </div>
                                <p style="font-size:0.7rem; font-weight:700; margin-bottom:4px; opacity:0.9;">Robust (corners tighten, circles survive)</p>
                                <div style="position: relative;">
                                <pre class="doc-pre-sm" id="so-radius-robust"><code>.app :where([class*="rounded-"]:not([class*="rounded-full"])) {
  border-radius: 2.5px;
}
::selection {
  background: #FFFF00;
  color: #000;
                                    ...
                                    <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('so-radius-robust').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                                </div>
                                <p style="margin-top: 12px; font-size: 0.7rem; color: var(--text-muted); line-height: 1.5;"><b>Note:</b> there is no <code>#app</code> element in Open WebUI &mdash; it's <code>.app</code> (a class). Selectors targeting <code>#app</code> silently do nothing. See <b>Section 13: Structural Transparency</b> for the full container/portal layout.</p>
                            </div>
                        </details>
                    </div>
                </details>
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        ...
                        <ul>
                            <li><b>Background Worker Execution:</b> By default, scripts run in a true background Web Worker utilizing <code>OffscreenCanvas</code> (if supported by your browser) to ensure high-performance rendering without blocking the UI thread. Both <b>Canvas 2D</b> and <b>WebGL/WebGL2</b> contexts are supported &mdash; call <code>canvas.getContext('2d')</code> for standard drawing or <code>canvas.getContext('webgl2')</code> for GPU-accelerated shader rendering (see <i>WebGL Rendering</i> below). Scripts that reference <code>document</code> are automatically detected and routed to the main thread &mdash; the status badge in the designer UI shows the <i>predicted</i> execution mode (derived from browser capability plus a static scan of the script source for <code>document</code>): <b style="color:#10b981">Background Worker</b> (OffscreenCanvas), <b style="color:#60a5fa">Main Thread (DOM Access)</b> (script uses <code>document</code>), or <b style="color:#f59e0b">Main Thread (Fallback)</b> (browser lacks <code>OffscreenCanvas</code>). If a script throws a runtime error in the Worker for any other reason, the engine also falls back to main-thread execution automatically &mdash; the badge does not update in that case, since it reflects the pre-launch prediction rather than the live runtime path.</li>
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-heartbeat-code"><code>setInterval(() =&gt; {
  self.postMessage({ type: 'heartbeat' });
}, 1000);</code></pre>
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-heartbeat-code').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-minimal-template-code"><code>let canvas, ctx, width, height;
...
function animate() {
  ...
  requestAnimationFrame(animate);
                            ...
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-minimal-template-code').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>
                            </div>
                        </details>
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-dom-observer-payload"><code>{ type: 'context', messages: 8, chars: 12400, estimatedTokens: 3100 }</code></pre>
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-dom-observer-payload').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>
                        <ul>
                        ...
                        <p style="margin: 12px 0 4px; font-weight: 600; font-size: 0.72rem; color: var(--text-main);">2. Socket Interceptor (exact, after each response)</p>
                        <p>The engine intercepts WebSocket messages from Open WebUI&rsquo;s Socket.IO connection. When a <code>chat:completion</code> event includes <code>usage</code> data (requires the <b>Usage</b> capability enabled on the model), the exact token counts are forwarded:</p>
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-socket-interceptor-payload"><code>{ type: 'context', exactTokens: 137, promptTokens: 48, completionTokens: 89, source: 'socket' }</code></pre>
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-socket-interceptor-payload').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>
                        <ul>
                            <li><code>exactTokens</code> &mdash; <code>usage.total_tokens</code>, or the prompt + completion sum when <code>total_tokens</code> is absent</li>
                            <li><code>promptTokens</code> / <code>completionTokens</code> &mdash; exact breakdown (supports OpenAI-style <code>prompt_tokens</code>/<code>completion_tokens</code> and Anthropic-style <code>input_tokens</code>/<code>output_tokens</code> field names)</li>
                            <li><code>source</code> &mdash; always <code>'socket'</code> to distinguish from DOM estimates</li>
                        </ul>
                        <p>Your script receives both. The DOM observer provides <b>continuous</b> updates during streaming (character count grows in real-time), while the socket interceptor provides an <b>exact snapshot</b> after each response completes. Prefer <code>exactTokens</code> when available.</p>
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-growth-animation-code"><code>const MAX_TOKENS = 128000;   // adjust to your model's context limit
...
if (contextRatio &lt; 0.1) { /* seed / empty state */ }
                            ...
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-growth-animation-code').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-custom-event-api-code"><code>// From any script running on the Open WebUI page:
window.dispatchEvent(new CustomEvent('owui-canvas-context', {
  detail: {
    messages: 12,
    chars: 48000,
    ...
    maxContext: 128000
                            ...
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-custom-event-api-code').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>
                        ...
                        <details class="doc-sub-accordion">
                            <summary>WebGL Rendering <i data-icon="chevron"></i></summary>
                            <div class="doc-sub-inner">
                        <p>Canvas FX scripts are not limited to the 2D Canvas API. Since <code>OffscreenCanvas</code> supports <b>WebGL and WebGL2</b> contexts, your scripts can run <b>full GLSL fragment shaders on the GPU</b> &mdash; enabling effects that are impossible or impractical with 2D canvas: raymarching, domain-warped fractal noise, volumetric lighting, metallic reflections, and real-time fluid simulations.</p>

                        <p class="doc-subheading">Why WebGL?</p>
                        <table class="doc-table-compact">
                            <thead>
                                <tr style="border-bottom: 1px solid var(--border);">
                                    <th></th>
                            ...
                            <tbody>
                                <tr style="border-bottom: 1px solid var(--border);">
                                    ...
                                    <td>GPU, massively parallel</td>
                                </tr>
                                <tr style="border-bottom: 1px solid var(--border);">
                                    <td><b>Per-pixel math</b></td>
                                    <td><code>getImageData</code> &mdash; slow</td>
                                    <td>Fragment shader &mdash; native</td>
                                </tr>
                                <tr style="border-bottom: 1px solid var(--border);">
                                    <td><b>Noise / fractals</b></td>
                                    <td>~10fps for 6-octave FBM</td>
                                    <td>60fps trivially</td>
                                </tr>
                                <tr style="border-bottom: 1px solid var(--border);">
                                    <td><b>Effects possible</b></td>
                                    <td>Particles, lines, fills</td>
                                    <td>Raymarching, volumetrics, fluid sim, SDF</td>
                                </tr>
                            </tbody>
                        </table>

                        <p class="doc-subheading">Minimal WebGL Template</p>
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-webgl-template-code"><code>var canvas, gl, program;
var uTime, uRes, uMouse;
var startTime, w, h;
...
function initGL() {
  ...
  // Fullscreen triangle (covers viewport with one triangle)
...
self.onmessage = function(e) {
  switch (e.data.type) {
    ...
    case 'mousemove':
      mouseNX = e.data.x / w;
      mouseNY = 1.0 - (e.data.y / h); // flip Y for GL
      break;
  }
};</code></pre>
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-webgl-template-code').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>

                        <p class="doc-subheading">Key Differences from 2D Scripts</p>
                        <ul>
                            <li><b>Context:</b> Call <code>canvas.getContext('webgl2')</code> with a <code>'webgl'</code> fallback instead of <code>'2d'</code>.</li>
                            <li><b>Drawing:</b> All rendering happens in GLSL fragment shaders compiled and linked into a WebGL program. Build shader source as an array of strings joined with <code>'\n'</code>.</li>
                            <li><b>Geometry:</b> Use the <b>fullscreen triangle trick</b> &mdash; a single triangle with vertices <code>[-1,-1, 3,-1, -1,3]</code> covers the entire viewport without quad seam artifacts.</li>
                            <li><b>Uniforms:</b> Pass <code>u_time</code> (float), <code>u_res</code> (vec2), and <code>u_mouse</code> (vec2, normalized 0&ndash;1) to the shader. Flip mouse Y for GL coordinates: <code>1.0 - (e.data.y / h)</code>.</li>
                            <li><b>No DOM:</b> WebGL runs natively in Workers &mdash; no DOM access needed. The same script works in both Worker and fallback modes.</li>
                            <li><b>ShaderToy compatible:</b> Any ShaderToy fragment shader can be adapted by mapping <code>iTime</code>&rarr;<code>u_time</code>, <code>iResolution</code>&rarr;<code>u_res</code>, <code>iMouse</code>&rarr;<code>u_mouse</code>, and <code>fragColor</code>&rarr;<code>gl_FragColor</code>.</li>
                        </ul>

                        <p class="doc-subheading">Shader Techniques Reference</p>
                        <ul>
                            <li><b>FBM (Fractal Brownian Motion):</b> Layer 5&ndash;6 octaves of noise for organic detail. Use domain warping (feeding one noise result into another) for deep organic flow.</li>
                            <li><b>Iq Color Palette:</b> <code>a + b * cos(6.28 * (c*t + d))</code> generates smooth, curated color gradients from four vec3 parameters.</li>
                            <li><b>Surface Normals:</b> Estimate normals from a heightfield by sampling 3 nearby points, enabling specular highlights, rim lighting, and fake environment reflections.</li>
                            <li><b>Voronoi:</b> Find nearest/second-nearest seed points for crystalline cell patterns. Multiply two offset Voronoi grids to create underwater caustic patterns.</li>
                            <li><b>Vignette:</b> <code>1.0 - dot(uv - 0.5, uv - 0.5) * strength</code> darkens edges for a natural focal effect.</li>
                        </ul>
                            </div>
                        </details>
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        ...
                        <ul>
                            ...
                            <li><b>Global Reset:</b> Resets all 4 modes to factory defaults &mdash; OKLCH sliders, variable overrides, locks, Custom CSS, Canvas FX, Manual Variable Overrides, and Gradient Background. Saved snapshots and preset libraries are kept. A confirmation dialog gives you the option to create a backup snapshot first.</li>
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        ...
                        <ul>
                            ...
                            <li><b>Visual Indicators:</b> Hovering over a theme card displays a rich tooltip showing the name, version, author, description, and color-coded feature tags: <span style="color:#6ee7b7;">Custom CSS</span>, <span style="color:#fdba74;">Canvas FX</span>, <span style="color:#f9a8d4;">Gradient</span>, <span style="color:#c4b5fd;">Overrides</span>, and <span style="color:#67e8f9;">Linked</span> (URL-linked themes).</li>
                            <li><b>Search:</b> Click the 🔍 icon to search themes by name, author, or description. Press Escape to close the search bar.</li>
                            <li><b>Tag Filter:</b> Click the filter icon to narrow the gallery by feature &mdash; show only themes that contain Custom CSS, Canvas FX, Overrides, or those that are Linked to an update URL.</li>
                            ...
                            <li><b>Duplicate Detection:</b> When importing themes (via file, URL, or clipboard), the tool automatically checks for duplicates and skips identical themes already in your gallery.</li>
                        </ul>
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        <p>The <b>Gradient</b> tab provides a dedicated visual gradient builder for applying CSS gradient backgrounds to your Open WebUI instance. The builder automatically generates all required CSS — including structural transparency rules, sidebar backdrop blur, and chat overlay blending — so you don't need to write any CSS manually.</p>
                        ...
                        <details class="doc-sub-accordion">
                            ...
                            <div class="doc-sub-inner">
                        <p>When the gradient is enabled (with at least 2 color stops), the designer automatically generates a <i>Structural Layer</i> that makes all layout containers transparent so the gradient shows through. It also adds a semi-opaque backdrop-blur rule to the sidebar for readability, and overrides the chat overlay for proper blending.</p>
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        ...
                        <details class="doc-sub-accordion">
                            <summary>The 6 Rendering Layers (Back to Front) <i data-icon="chevron"></i></summary>
                            <div class="doc-sub-inner">
                        <p>When Canvas FX and/or Gradient Background are active, the final visual output is composed of 6 distinct layers stacked using CSS <code>z-index</code> and <code>position: fixed</code>:</p>
                        <div class="doc-table-wrap">
                            <table class="doc-table">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <tr><td><b>4. Structural Transparency</b></td><td>—</td><td>CSS rules that set <code>background-color: transparent</code> on <code>.app</code>, <code>.app&nbsp;&gt;&nbsp;div</code>, <code>main</code>, <code>nav</code>, <code>.sticky</code>, and <code>[class*="bg-gray-"]</code> layout wrappers — but explicitly <b>excludes</b> interactive elements (<code>button</code>, <code>a</code>, <code>input</code>, <code>select</code>, <code>label</code>, <code>span</code>) via <code>:not()</code> filters. Uses CSS <code>:where()</code> for zero-specificity transparency, so any normal-specificity rule can override it. See <i>Overlay UI Protection</i> below for how portaled overlay UI stays opaque.</td></tr>
                                    <tr><td><b>5. Glass Panels</b></td><td>&mdash;</td><td>A semi-opaque overlay with <code>backdrop-filter: blur()</code> on the <b>sidebar</b>, keeping navigation readable while letting the background show through. The Gradient Builder auto-calculates the RGBA value from the darkest gradient stop (or the mesh background color for Mesh gradients). The sidebar&rsquo;s glass behavior is controlled by the <b>Sidebar Transparency</b> valve: <code>opaque</code> (default) uses a solid background, <code>translucent</code> applies a tinted glass effect with <code>backdrop-filter: blur(20px) saturate(1.3)</code>, and <code>transparent</code> removes the background entirely (auto-upgraded to translucent on mobile for readability). Both sidebar and overlay transparency valves work independently of Canvas FX and gradient state.</td></tr>
                                    <tr><td><b>6. Overlay Glass</b></td><td>&mdash;</td><td>Portaled UI elements (settings modal, dropdown menus, user menus, integration menus) that live outside <code>.app</code> on <code>&lt;body&gt;</code>. Controlled by the <b>Overlay Transparency</b> valve: <code>opaque</code> (default) keeps solid backgrounds for readability, <code>translucent</code> applies frosted glass with <code>backdrop-filter: blur(20px) saturate(1.3)</code>. Uses CSS <code>:not(.app *)</code> exclusions to avoid affecting elements inside the main app container.</td></tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <p>If neither is active, the designer emits simple opaque <code>background-color</code> rules for <code>body</code> and <code>#sidebar</code> only — no transparency is applied, and textareas keep Open WebUI's native styling (recolored by the palette variables) so intentionally-transparent fields are left alone.</p>
                        ...
                        <ol style="font-size: 0.72rem; line-height: 1.7; padding-left: 18px;">
                            ...
                            <li><b>Structural Background Rules</b> — when Canvas FX or a gradient is active, transparency rules on the app containers plus <code>body</code>/<code>#sidebar</code> backdrops; otherwise just opaque <code>body</code>/<code>#sidebar</code> rules</li>
                            <li><b>Gradient Background CSS</b> — <code>background-image</code>, sidebar blur, and chat overlay rules</li>
                        ...
                        <div class="doc-table-wrap">
                            <table class="doc-table">
                                ...
                                <tbody>
                                    ...
                                    <tr><td><span class="doc-code-inline">Ctrl+Y</span> / <span class="doc-code-inline">Ctrl+Shift+Z</span></td><td>Redo last undone change</td></tr>
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        ...
                        <div class="doc-subheading">🎭 Built-in Community Themes Browser</div>
                        <p>The <b>Themes</b> area includes a curated <b>Community Themes</b> grid, fetched from the presets repository's <code>manifest.json</code> on GitHub. Each card shows the theme's preview colors and metadata with a one-click <b>Install</b> button — installed themes are marked, and if the remote version is newer than your installed copy, the card offers an update. An <b>Install All</b> button appears when multiple themes are uninstalled, alongside <b>Refresh</b>, search, tag-filter, and sort controls. The whole browser can be hidden via the <b>Enable Community Themes</b> valve (which also skips the manifest fetch entirely).</p>
                ...
                <details class="doc-accordion">
                    ...
                    <div class="doc-inner">
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-nginx-config"><code>location / {
    ...
    proxy_buffering off;
                            ...
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-nginx-config').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>
                        ...
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-caddy-config"><code>your-domain.com {
    reverse_proxy localhost:8080 {
        flush_interval -1
    }
}</code></pre>
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-caddy-config').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>

                        <div class="doc-subheading">Apache</div>
                        <p>Disable output buffering for the Open WebUI proxy:</p>
                        <div style="position: relative;">
                        <pre class="doc-pre" id="doc-apache-config"><code>ProxyPass / http://127.0.0.1:8080/
ProxyPassReverse / http://127.0.0.1:8080/

# Disable buffering for SSE streaming
SetEnv proxy-sendchunked 1
SetEnv proxy-sendcl 0</code></pre>
                            <button class="owui-code-btn" style="position: absolute; top: 6px; right: 6px; font-size: 0.6rem;" onclick="navigator.clipboard.writeText(document.getElementById('doc-apache-config').textContent).then(()=>showToast('Copied to clipboard'))">Copy</button>
                        </div>
...
location.reload();</code></pre>
                        ...
                        <ul>
                            ...
                            <li><code>DATA_DIR/theme/open_theme_designer_sections.json</code></li>
                ...
                <details class="doc-accordion" open>
                    ...
                    <div class="doc-inner" style="display:flex; flex-direction:column; align-items:center; gap:16px;">
                        <p style="margin:0; text-align:center;">Export your complete theme gallery snapshots, custom CSS presets, canvas animation scripts, and gradient presets to separate portable backup files at once. You can restore them anytime using the import features in their respective tabs.</p>
                ...
                <details class="doc-accordion" open>
                    ...
                    <div class="doc-inner" style="display:flex; flex-direction:column; align-items:center; gap:16px;">
                        <p style="margin:0; text-align:center; color:var(--text-muted); font-size:0.75rem; line-height:1.6;">Permanently wipe <b>all</b> Theme Designer Pro data including your theme gallery, CSS snippets, canvas scripts, gradient presets, and active configuration. This action cannot be undone.</p>
    ...
    <div class="footer">
        <div class="footer-left">
            ...
            <label id="sync-all-toggle-wrap" class="sync-all-toggle" style="display:none;" data-tooltip="When enabled, loading presets and syncing applies to all 4 modes automatically">
                <input type="checkbox" id="sync-all-modes-toggle" class="cb-input">
                <span style="font-size:0.6rem; font-weight:600; opacity:0.85; white-space:nowrap;">Sync All</span>
            </label>
    ...
    <div id="update-modal" class="modal-overlay z-high">
        <div class="control-group modal-panel modal-panel-neutral" style="width: 480px; max-width: 90vw; max-height: 85vh; display: flex; flex-direction: column;">
            ...
            <div id="update-diff-panel" style="display: none; flex: 1; min-height: 0; overflow: hidden; margin-bottom: 12px;"></div>
            <div class="flex-end" style="flex-shrink: 0;">
                <button class="btn" id="update-skip-btn">Cancel</button>
                <button class="btn" id="update-ignore-btn" style="color: var(--text-muted);">Ignore This Update</button>
...
<script>
    ...
    const GALLERY_TOOLBARS = [
        {
            mountId: 'gallery-toolbar-theme', title: 'Theme Gallery',
            ...
            scrollId: 'snapshot-scroll', scrollStyle: 'max-height: 35vh; overflow-y: auto; overflow-x: hidden; padding: 12px 8px; margin: -12px -8px 12px -8px; width: calc(100% + 16px); display: none;',
        ...
        },
        {
            ...
            filter: { btnId: 'canvas-filter-toggle-btn', dropdownId: 'canvas-filter-dropdown', tooltip: 'Filter by Type', options: [
                { value: 'all', label: 'All Scripts' }, { value: 'webgl', label: 'WebGL' }, { value: 'context', label: 'Context' }, { value: 'dom', label: 'DOM Access' }
            ]},
            ...
            scrollId: 'canvas-preset-scroll', scrollStyle: 'max-height: 600px; overflow-y: auto; overflow-x: hidden; padding: 12px 8px; margin: -12px -8px 16px -8px; width: calc(100% + 16px); display: none;',
    ...
    function buildGalleryToolbar(cfg) {
        ...
        return `<div class="section-title" style="margin-top: 8px; display: flex; align-items: center; gap: 8px;">` +
            ...
            `<div class="flex-center-gap6">` +
                `<div class="search-wrap"><i class="search-icon-overlay">${ICONS.search}</i><input type="text" id="${cfg.searchId}" placeholder="${cfg.searchPlaceholder}" class="search-input-expand" /><button type="button" class="search-clear-btn" id="${cfg.searchId}-clear" aria-label="Clear search">&times;</button></div>` +
    ...
    const Storage = {
        ...
        // exceeded, storage disabled). Callers that batch writes (e.g. the
        // "Install All" import loop) can check the result and stop cleanly
        // instead of throwing mid-operation and corrupting state.
        set(key, value) { return this.setRaw(key, JSON.stringify(value)); },
        getRaw(key) { return this._store(key).getItem(this._resolve(key)); },
        setRaw(key, value) {
            try {
                this._store(key).setItem(this._resolve(key), value);
                return true;
            } catch (e) {
                // QuotaExceededError (name varies across browsers) or storage
                // unavailable (private mode). Warn the user once per write —
                // silent failure would look like data loss.
                console.warn('[Theme Pro] Storage write failed for', key, e);
                if (typeof showToast === 'function') {
                    showToast('⚠️ Browser storage full — export and remove old snapshots to free space');
                }
                return false;
            }
        },
    ...
    function setDraftMode(on) {
        if (on && !_draftMode) {
            ...
            // Skip seeding if sessionStorage already has data (preserves draft
            // state across page refresh — the previous session's writes survive).
            _BOOTLOADER_WATCHED_KEYS.forEach(key => {
                const resolved = Storage._resolve(key);
                if (!sessionStorage.getItem(resolved)) {
    ...
    function publishTheme() {
        ...
        // Regenerate CSS + sections from themeData rather than reusing the
        // css_cache blob: the server needs matching structured sections with
        // every CSS save (a css-only POST deletes the sections file as stale).
        const sections = generateThemeSections();
        const css = assembleThemeCss(sections);
        const state = Storage.getRaw('theme') || '{}';
        fetch('{ROUTE_BASE}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ css, state, sections })
        }).then(r => {
            if (r.ok) {
                showToast('Published! Theme pushed to all users.');
                setDraftMode(false);
                // Mirror syncToServer: persist the valve-postprocessed CSS into
                // the localStorage fallback now that draft mode is off (Storage
                // routes css_cache back to localStorage after setDraftMode).
                r.json().then(d => {
    ...
    function syncToServer(css, sections) {
        ...
        _syncTimer = setTimeout(() => {
            ...
            // leave stale CSS paired with fresh state (old gradient + new
            // canvas) and broadcast that mix to every client. The sequence
            // number keeps a superseded response from overwriting css_cache.
            ...
            _syncQueue = _syncQueue.then(() => fetch('{ROUTE_BASE}', {
                ...
                if (r.ok) {
                    ...
                    // Sole localStorage write for live edits (injectLive defers
                    // to here): persist the valve-postprocessed CSS the server
                    // returns — the same CSS every other client receives — so
                    // other tabs' 'storage' listeners and the Watchtower-recovery
                    // fallback never see the raw pre-processed copy (which has
                    // opaque sidebar rules → visible flash). When the server
                    // omits css, post-processing changed nothing: raw is exact.
                    ...
                    // Keep the recovery fallback fresh even when sync fails —
                    // with the server down, clients fall back to localStorage
                    // anyway, and raw CSS beats a stale theme.
                    if (mySeq === _syncSeq) { try { Storage.setRaw('css_cache', css); } catch(x) {} }
                }
            }).catch(e => {
                console.warn('[Theme Pro] Server sync error:', e);
                showToast('⚠️ Server unreachable — changes may not be saved');
                if (mySeq === _syncSeq) { try { Storage.setRaw('css_cache', css); } catch(x) {} }
            }));
        }, 250);
    }
    ...
    function syncLibrary() {
        ...
        _libTimer = setTimeout(() => {
            ...
            fetch('{ROUTE_BASE}', {
                ...
                console.warn('[Theme Pro] Library sync error:', e);
                showToast('⚠️ Library sync failed — presets may not be saved');
            });
        }, 500);
    }
    ...
    function downloadAllBackups() {
        ...
        let count = 0;
        if (snapshots.length > 0) { downloadFile({ themes: snapshots, isLibraryBackup: true, version: "{VERSION}" }, `owui-themes-backup-${dateStr}.json`); count++; }
        if (CSS_PRESETS.length > 0) { downloadFile({ css_presets: CSS_PRESETS, isCssBackup: true, version: "{VERSION}" }, `owui-css-backup-${dateStr}.json`); count++; }
        if (CANVAS_PRESETS.length > 0) { downloadFile({ canvas_presets: CANVAS_PRESETS, isCanvasBackup: true, version: "{VERSION}" }, `owui-canvas-backup-${dateStr}.json`); count++; }
        if (CUSTOM_GRADIENT_PRESETS.length > 0) { downloadFile({ gradient_presets: CUSTOM_GRADIENT_PRESETS, isGradientBackup: true, version: "{VERSION}" }, `owui-gradient-presets-backup-${dateStr}.json`); count++; }
        return count;
    }
    ...
    function buildModals() {
        ...
        function buildConfirm(d) {
            ...
            return `<div id="${d.id}" class="modal-overlay${zCls}" role="dialog" aria-modal="true" aria-labelledby="${d.id}-title"><div class="control-group modal-panel${v.panel}">${icon}<div class="section-title" id="${d.id}-title" style="color:${v.tc}; justify-content:center; margin-bottom: 8px;">${d.title}</div><p${msgId} style="font-size: 0.75rem; color:var(--text-muted); margin-bottom:${mb}; line-height:1.6;${extra}">${d.msg}</p>${d.extraHtml || ''}${cbHtml}<div style="display:flex; flex-wrap:wrap; gap:12px;${center}"><button class="btn" style="flex:1; min-width:110px;" ${close(d.id)}>${d.cancelLabel || 'Cancel'}</button>${d.secondaryId ? `<button class="btn" id="${d.secondaryId}" style="flex:1; min-width:110px;">${d.secondaryLabel}</button>` : ''}<button class="${bCls}" id="${d.confirmId}" style="${bStyle}">${d.confirmLabel}</button></div></div></div>`;
        }
        ...
        const defs = [
            ...
            // ── Confirm: Mass delete ──
            { t:'c', id:'delete-all-themes-modal', icon:'!', title:'Wipe Theme Gallery?', msg:'Are you sure you want to permanently remove <b>ALL</b> saved themes from your gallery?', confirmId:'confirm-delete-all-themes-btn', confirmLabel:'Wipe All' },
            ...
            { t:'c', id:'factory-reset-modal', zClass:'z-top', icon:'\u{1F6A8}', title:'Factory Reset?', msg:'This will permanently WIPE all themes, snapshots, and settings. This action is irreversible.', msgMb:'20px', confirmId:'confirm-factory-reset-btn', confirmLabel:'Wipe All', cb:{ id:'factory-backup-cb', label:'Export a backup before wiping', checked:true } },
            ...
            { t:'c', id:'export-all-backups-modal', zClass:'z-top', variant:'info', icon:'\u{1F4E6}', title:'Export All Backups', msgId:'export-all-backups-desc', msg:'This will download separate <b>.json</b> backup files for your <b>Theme Gallery</b>, <b>CSS Snippets</b>, <b>Canvas Scripts</b>, and <b>Gradient Presets</b> (if any exist).', confirmId:'confirm-export-all-backups-btn', confirmLabel:'Export' },
            { t:'c', id:'import-clipboard-modal', variant:'info', icon:'\u{1F4CB}', title:'Import from Clipboard?', msg:"We've detected a theme configuration on your clipboard. Would you like to import and apply it now?", msgMb:'20px', confirmId:'confirm-clipboard-import-btn', confirmLabel:'Import & Apply', btnCls:'btn btn-primary', btnStyle:'flex:1; min-width:110px;', secondaryId:'clipboard-import-noapply-btn', secondaryLabel:'Import Without Applying', extraHtml:canvasWarning },
...
function startAnimation() {
    ...
    const saved = (() => {
        if (_resumingDraft) {
            // Read draft state directly from sessionStorage. We can't use
            // Storage.get() here because _draftMode is still false at this
            // point, so it would route to localStorage (published state).
            try {
                const draftRaw = sessionStorage.getItem(Storage._resolve('theme'));
    ...
    function updatePalette() {
        ...
        const webglBadge = $('canvas-webgl-badge');
        const isActiveCanvas = config.canvasEnabled && config.canvasScript && config.canvasScript.trim() !== '';
        const scriptTags = getCanvasScriptTags(config.canvasScript);
        if (webglBadge) {
            webglBadge.style.display = (isActiveCanvas && scriptTags.webgl) ? 'inline-block' : 'none';
        }
        const contextBadge = $('canvas-context-badge');
        if (contextBadge) {
            contextBadge.style.display = (isActiveCanvas && scriptTags.context) ? 'inline-block' : 'none';
        }
        if (workerBadge) {
            if (isActiveCanvas) {
                ...
                if (typeof HTMLCanvasElement !== 'undefined' && !!HTMLCanvasElement.prototype.transferControlToOffscreen) {
                    if (scriptTags.dom) {
                        workerBadge.innerText = 'Main Thread (DOM Access)';
                        workerBadge.style.color = '#60a5fa';
                        workerBadge.style.backgroundColor = 'rgba(96, 165, 250, 0.1)';
                    } else {
        ...
        // Generate the CSS sections once and pass to all consumers (was generated
        // 3x per change). Pass pre-computed activeVars for the current mode so
        // generateThemeSections() can skip recomputing getVariablesMap() for it.
        const _sections = generateThemeSections({ [dm]: activeVars });
        const _css = assembleThemeCss(_sections);
        updateCodeView(_css);
        if (shouldInject) injectLive(_css, _sections);
        else forceLocalIframeTheme(_sections);
    ...
    function renderPresetGallery({ presets, galleryId, scrollAreaId, searchInputId, countWrapId, countId,
        sortVar, typeFilter = 'all', typeMatch = null, contentField, activeRefMap, loadFnName, emptyMsg, noResultsPrefix,
        ...
        const typeFiltering = typeFilter && typeFilter !== 'all' && typeof typeMatch === 'function';
        let filtered = presets.map((p, i) => ({ ...p, _origIndex: i }));
        if (searchQuery) {
            filtered = filtered.filter(p => (p.name || '').toLowerCase().includes(searchQuery));
        }
        if (typeFiltering) {
            filtered = filtered.filter(p => typeMatch(p[contentField], typeFilter));
        }
        if (sortVar !== 'default') {
            filtered.sort((a, b) => {
        ...
        if (countEl) countEl.innerText = (searchQuery || typeFiltering) ? filtered.length + '/' + presets.length : presets.length;
        ...
        if (filtered.length === 0) {
            ...
            const noResultsMsg = searchQuery
                ? `${noResultsPrefix} &ldquo;${_esc(searchQuery)}&rdquo;`
                : `${noResultsPrefix} this filter`;
            gallery.innerHTML = `<div class="empty-state">${noResultsMsg}</div>`;
            return;
        }
    ...
    // Detect capability "badges" from a Canvas FX script's source. Shared by the
    // editor status badges and the Preset Gallery type filter so both stay in sync.
    function getCanvasScriptTags(script) {
        const s = script || '';
        return {
            webgl: /getContext\s*\(\s*['"]webgl/.test(s),
            context: /(?:case|===)\s*['"]context['"]/.test(s),
            dom: s.indexOf('document') !== -1,
        };
    }
    ...
    function renderCanvasPresets() {
        renderPresetGallery({
            ...
            typeFilter: typeof activeCanvasTypeFilter !== 'undefined' ? activeCanvasTypeFilter : 'all',
            typeMatch: (script, f) => !!getCanvasScriptTags(script)[f],
    ...
    // Module-level reference to the latest computed variables map.
    // Updated by renderVariableGrid() on each call so the delegated
    // lock handler can read the current computed color for any step.
    let _lastActiveVars = {};

    // Attach delegated event listeners to the variable grid container ONCE.
    // Called at init time — NOT on every updatePalette() cycle.
    function setupVariableGridDelegation() {
        const grid = document.getElementById('variable-grid');
        if (!grid) return;

        // --- Color picker input (live preview while dragging) ---
        grid.addEventListener('input', (e) => {
            const picker = e.target.closest('.var-picker');
            if (!picker) return;
            const dm = getActiveDataMode();
            const varName = picker.dataset.var;
            themeData[dm].overrides[varName] = picker.value;
            themeData[dm].locks[varName] = true;
            commitChange({ clearRef: false, push: false });
        });

        // --- Color picker change (final value — push undo state) ---
        grid.addEventListener('change', (e) => {
            if (!e.target.closest('.var-picker')) return;
            pushState();
        });

        // --- Click delegation for lock + reset buttons ---
        grid.addEventListener('click', (e) => {
            // Lock toggle
            const lock = e.target.closest('.var-lock');
            if (lock) {
                const dm = getActiveDataMode();
                const varName = lock.dataset.var;
                ...
                if (isLocked) {
                    ...
                    themeData[dm].locks[varName] = true;
                    themeData[dm].overrides[varName] = _lastActiveVars[varName];
                }
            ...
            // Reset to default
            const reset = e.target.closest('.var-reset');
            if (reset) {
                const varName = reset.dataset.var;
                const dm = getActiveDataMode();
    ...
    function renderVariableGrid(config, activeVars) {
        ...
        // Update module-level ref so delegated lock handler sees current values
        _lastActiveVars = activeVars;
    ...
    // Assemble the flat CSS blob (Code tab, /theme.css, localStorage cache) from
    // structured sections. Each section string carries its own /*[OWUI_*]*/ markers,
    // so plain concatenation reproduces the legacy flat format byte-for-byte.
    // 'extra' holds server-side valve-appended rules and is empty client-side.
    function assembleThemeCss(s) {
        return s.vars + s.structural + s.gradient + s.custom + (s.extra || '');
    }

    function generateCoreCSS(varsCache) {
        return assembleThemeCss(generateThemeSections(varsCache));
    }

    // Generate the theme CSS as structured sections { vars, structural, gradient, custom }.
    // Consumers pick the sections they need instead of regex-slicing the flat blob:
    // the designer page uses vars+custom, the iframe self-theme uses vars only, and
    // the bootloader assembles auth-page CSS from the server-delivered copy.
    function generateThemeSections(varsCache) {
        // varsCache: optional { modeId: varsMap } — pre-computed vars from updatePalette()
        // to avoid recomputing getVariablesMap() for the active mode.
...
`;
        ...
        const wrapSystemMedia = (modeId, contentFn) => {
            ...
            const inner = contentFn('html[data-theme="system"], :root[data-theme="system"]');
            if (!inner || !inner.trim()) return ''; // Skip empty @media shells (feature disabled for mode)
        ...
        let varsCss = `/*[OWUI_VARS_START]*/\n`;
        modes.forEach(m => {
            const modeTag = m.id.toUpperCase();
            const modeData = themeData[m.id];

            // When palette is disabled, emit the mode's default values
            // so OLED still gets darker defaults (l=0) and other modes
            // match OWUI's @theme exactly. This avoids stripping palette
            // markers which would leave no mode-specific vars at all.
            let vars;
            if (modeData.paletteEnabled === false) {
                vars = {};
                steps.forEach(step => {
                    vars[`--color-gray-${step}`] = getDefaultColorForStep(m.id, step);
                });
            } else {
                vars = (varsCache && varsCache[m.id]) || getVariablesMap(modeData, m.id);
            }

            // Palette variables
            varsCss += `/*[OWUI_PAL_${modeTag}_START]*/\n`;
            varsCss += `/* --- ${m.name} VARIABLES --- */\n${m.sel} {\n`;
            Object.entries(vars).forEach(([k, v]) => { varsCss += `  ${k}: ${v} !important;\n`; });
            varsCss += `}\n`;
            varsCss += wrapSystemMedia(m.id, (sel) => {
                let inner = `  ${sel} {\n`;
            ...
            if (modeData.manualOverrides && modeData.manualOverrides.trim() && !!modeData.manualOverridesEnabled) {
                ...
                varsCss += `/* --- ${m.name} MANUAL OVERRIDES --- */\n${m.sel} {\n`;
                lines.forEach(line => { varsCss += formatOverrideLine(line); });
                varsCss += `}\n`;
                varsCss += wrapSystemMedia(m.id, (sel) => {
                    let inner = `  ${sel} {\n`;
        ...
        varsCss += `/*[OWUI_VARS_END]*/\n\n`;

        // Structural, gradient, and custom CSS — single pass, separate sections
        let structuralCss = `/*[OWUI_STRUCTURAL_START]*/\n`;
        let gradientCss = '';
        let customCss = '';
        modes.forEach(m => {
            const modeData = themeData[m.id];
        ...
        structuralCss += `/*[OWUI_STRUCTURAL_END]*/\n`;

        return { vars: varsCss, structural: structuralCss, gradient: gradientCss, custom: customCss };
    }
    ...
    function forceLocalIframeTheme(sections) {
        const dm = getActiveDataMode();
        const config = themeData[dm];
        const dataMode = dm;
        const activeVars = getVariablesMap(config);
        ...
        let darkBorder = washedOut.dark ? "rgba(0,0,0,0.2)" : "var(--color-gray-800, oklch(0.26 0 0))";

        let lightTextMain = washedOut.light ? "#18181b" : "var(--color-gray-950, #09090b)";
        let lightTextMuted = washedOut.light ? "#52525b" : "var(--color-gray-600, #52525b)";
        let lightBorder = washedOut.light ? "rgba(0,0,0,0.2)" : "var(--color-gray-200, #e4e4e7)";
        let lightBgElevated = washedOut.light ? "rgba(0,0,0,0.05)" : "var(--color-gray-100, #f4f4f5)";
        
        // Vars-only baseline for designer self-theming — the structural, gradient,
        // and custom sections target the main app layout and are omitted here.
        let iframeCss = (sections || generateThemeSections()).vars;

        // Palette vars for disabled modes now emit defaults
        // in generateThemeSections(), so no stripping needed here.

        // Build a targeted gradient rule for the designer page body
        let toolGradientCss = '';
        let toolNeedsTransparentContainer = false;
        try {
            const gradDm = getActiveDataMode();
            const gradConfig = themeData[gradDm];
            // Canvas FX or Gradient: make container transparent so body-level visuals show through
            if (gradConfig && gradConfig.canvasEnabled && gradConfig.canvasScript && gradConfig.canvasScript.trim()) {
                toolNeedsTransparentContainer = true;
            }
        ...
        iframeCss += `
            ...
            ${toolGradientCss}
            ${toolNeedsTransparentContainer ? `
            /* Canvas FX / Gradient: make wrapper transparent so body-level visuals show through */
            #theme-designer-container {
                background: transparent !important;
                isolation: auto !important;
            }
            /* Translucent panels so background effects bleed through */
            #tab-lch .control-group,
            #tab-bg .control-group,
            #tab-lch .var-item {
                background: rgba(0, 0, 0, 0.45) !important;
                backdrop-filter: blur(16px) !important;
                -webkit-backdrop-filter: blur(16px) !important;
            }
            body#tool-body.light-mode #tab-lch .control-group,
            body#tool-body.light-mode #tab-bg .control-group,
            body#tool-body.light-mode #tab-lch .var-item {
                background: rgba(255, 255, 255, 0.55) !important;
            }
            ` : ''}
            
            body#tool-body {
                --bg-deep: var(--color-gray-900, oklch(0.20 0 0)) !important;
                --bg-surface: var(--color-gray-850, oklch(0.23 0 0)) !important;
                --bg-elevated: var(--color-gray-800, oklch(0.26 0 0)) !important;
                --border: ${darkBorder} !important;
                --text-main: ${darkTextMain} !important;
                --text-muted: ${darkTextMuted} !important;
            }
            body#tool-body.light-mode {
                --bg-deep: var(--color-gray-50, #f4f4f5) !important;
        ...
        iframeStyle.textContent = iframeCss;
    ...
    function injectLive(preGeneratedCss, sections) {
        const s = sections || generateThemeSections();
        let css = preGeneratedCss || assembleThemeCss(s);
        try {
                ...
                if (shouldInject) {
                    ...
                    // On the designer page, omit the structural transparency and gradient
                    // sections from the live style element — they target body/.app/#theme-designer-container
                    // which collides with the designer's own layout. The designer page
                    // handles its own gradient preview via the separate iframe-live-theme
                    // style element with properly scoped body#tool-body selectors.
                    // The full CSS is still saved to localStorage + server below.
                    if (window.__THEME_DESIGNER__ || document.getElementById('theme-designer-container')) {
                        oldStyle.textContent = s.vars + s.custom;
                    } else {
                        oldStyle.textContent = css;
                    }
                    // Live mode: the cross-tab localStorage write is deferred to
                    // the POST response in syncToServer(). The server returns the
                    // valve-postprocessed CSS (sidebar/overlay transparency), and
                    // writing the raw CSS here would make other tabs' 'storage'
                    // listeners apply opaque /*[FX]*/ sidebar rules for a couple
                    // of seconds until the SSE refetch delivers the rewritten CSS.
                    // Draft mode still writes immediately — Storage routes it to
                    // sessionStorage (tab-scoped, invisible to other tabs).
                    if (_draftMode) Storage.setRaw('css_cache', css);
                    // Serialize state once — reused for both Storage and CustomEvent
                    // (previously serialized twice: once by Storage.set, once for the event)
                    ...
                    if (!document.getElementById('owui-theme-bootloader')) {
                        try {
                            ...
                            if (document.getElementById('theme-designer-container')) {
    ...
    function renderGradientPresets() {
        ...
        let html = allEntries.map(entry => {
            ...
            return `
                <div class="gradient-preset-chip ${isActive ? 'active' : ''}" onclick="${onclick}" data-tooltip="${escapedName}" style="position:relative;">
                    <div class="gradient-preset-swatch" style="${bgColorStyle}background-image: ${gradStr};"></div>${customActions}
                    <div class="gradient-preset-label">${_esc(entry.preset.name)}</div>
                    <div class="selected-check">✓</div>
                </div>`;
        }).join('');
    ...
    function updateSyncDeltaBadges() {
        ...
        for (const mode of targetModes) {
            ...
            // === Gradient ===
            const gradientSnapshot = (d) => ({ stops: d.gradientStops || [], type: d.gradientType || 'linear', angle: d.gradientAngle ?? 135, intensity: d.gradientIntensity ?? 85, anim: !!d.gradientAnimation, radialPosX: d.gradientRadialPosX ?? 50, radialPosY: d.gradientRadialPosY ?? 50, radialShape: d.gradientRadialShape || 'ellipse', radialSize: d.gradientRadialSize || 'farthest-corner', meshPoints: d.gradientMeshPoints || [], meshBgColor: d.gradientMeshBgColor || '#0a0a12' });
            const gMatch = compareGradientProps(sourceData, targetData);
            syncDiffData.gradient.push({
                mode, match: gMatch,
                src: gradientSnapshot(targetData),
                tgt: gradientSnapshot(sourceData)
            });
    ...
    function showSyncModal(sourceType, sourceId) {
        ...
        if (sourceType === 'active-mode') {
            ...
            descText = `Choose where and what to apply from <b>${_esc(name)}</b>.`;
        ...
        collapseAllSyncDiffs();
        updateSyncDeltaBadges();
        // Auto-sync: skip modal entirely
        const cfg = window.__THEME_PRO_CONFIG__;
        if (cfg && cfg.autoSync) {
            const dm = getActiveDataMode();
            const syncAllToggle = $('sync-all-modes-toggle');
            const syncAll = syncAllToggle && syncAllToggle.checked;

            if (sourceType === 'active-mode') {
                // Sync button: always syncs to OTHER modes (that's its purpose)
                const targetModes = MODES.filter(m => m !== dm);
                if (targetModes.length === 0) return;
                const srcData = themeData[dm];
                targetModes.forEach(mode => {
                    applySyncToMode(mode, srcData, {
                        syncPalette: true, syncOverrides: true, syncCss: true,
                        syncCanvas: true, syncGradient: true, syncAuth: true,
                        adjustL: (m, l) => m === 'oled' ? 0 : l,
                        cssRef: activeCssRef[dm],
                        canvasRef: activeCanvasRef[dm],
                        gradientRef: activeGradientRef[dm],
                    });
                    activeThemeRef[mode] = activeThemeRef[dm] ? { ...activeThemeRef[dm] } : null;
                });
                showToast('Theme synced to all modes!');

            } else if (sourceType === 'snapshot') {
                const snapshots = getSnapshots();
                const s = snapshots[sourceId];
                if (!s) return;
                ...
                if (syncAll) {
                    ...
                    MODES.forEach(mode => {
                        ...
                        activeThemeRef[mode] = { type: 'snapshot', id: sourceId };
                    });
                    showToast(`Theme "${s.name}" applied to all modes!`);
                } else {
                    // Apply to CURRENT mode only
                    let sourceModeData = s[dm];
                    if (!sourceModeData) {
                        if (dm === 'oled' && s.dark) { sourceModeData = structuredClone(s.dark); sourceModeData.l = 0; }
                        else if (dm === 'her' && s.light) { sourceModeData = structuredClone(s.light); }
                        else { sourceModeData = structuredClone(s.dark || s.light); }
                    }
                    applySyncToMode(dm, sourceModeData, {
                        syncPalette: true, syncOverrides: true, syncCss: true,
                        syncCanvas: true, syncGradient: true, syncAuth: true,
                        rebuildLocks: true,
                    });
                    activeThemeRef[dm] = { type: 'snapshot', id: sourceId };
    ...
    $('clipboard-import-noapply-btn').onclick = async () => {
        if (pendingClipboardData) {
            const file = new File([pendingClipboardData], "clipboard-import.json", { type: "application/json" });
            await handleFilesImport([file], { applyAfter: false });
            pendingClipboardData = null;
        }
        hideModal('import-clipboard-modal');
    };
    ...
    if (exportAllBackupsBtn) {
        ...
        document.getElementById('confirm-export-all-backups-btn').onclick = () => {
            ...
            const exportedCount = downloadAllBackups();
            if (exportedCount > 0) {
    ...
    setupMassDelete({
        ...
        emptyMsg: 'No themes to delete', toastMsg: 'Theme Gallery Cleared!',
    ...
    async function handleFilesImport(files, { skipDuplicateCheck = false, applyAfter = true } = {}) {
        ...
        if (importedCount > 0) {
            ...
            if (applyAfter) {
                ...
                // Import into the gallery without activating the theme.
                renderSnapshots();
            }
    ...
    const themeSearchClear = document.getElementById('theme-search-input-clear');
    let themeSearchOpen = false;

    function updateThemeClearBtn() {
        if (themeSearchClear) {
            if (themeSearchOpen && themeSearchInput.value.length > 0) {
                themeSearchClear.classList.add('visible');
            } else {
                themeSearchClear.classList.remove('visible');
            }
        }
    }
    ...
    if (themeSearchToggle && themeSearchInput) {
        ...
        themeSearchInput.addEventListener('input', () => { updateThemeClearBtn(); renderSnapshots(); });
    ...
    if (themeSearchClear && themeSearchInput) {
    ...
    // Canvas FX Type Filter (badge-based: WebGL / Context / DOM Access)
    let activeCanvasTypeFilter = 'all';
    const canvasFilterBtn = document.getElementById('canvas-filter-toggle-btn');
    const canvasFilterDropdown = document.getElementById('canvas-filter-dropdown');
    if (canvasFilterBtn && canvasFilterDropdown) {
        canvasFilterBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            canvasFilterDropdown.classList.toggle('open');
        });
        canvasFilterDropdown.querySelectorAll('.tag-filter-option').forEach(opt => {
            opt.addEventListener('click', (e) => {
                e.stopPropagation();
                activeCanvasTypeFilter = opt.getAttribute('data-filter');
                canvasFilterDropdown.querySelectorAll('.tag-filter-option').forEach(o => o.classList.remove('active'));
                opt.classList.add('active');
                canvasFilterBtn.classList.toggle('has-filter', activeCanvasTypeFilter !== 'all');
                canvasFilterDropdown.classList.remove('open');
                renderCanvasPresets();
            });
        });
        document.addEventListener('click', (e) => {
            if (!canvasFilterBtn.contains(e.target) && !canvasFilterDropdown.contains(e.target)) {
                canvasFilterDropdown.classList.remove('open');
            }
        });
    }
    ...
    function setupGallerySearch(toggleId, inputId, renderFn) {
        ...
        if (toggle && input) {
            ...
            input.addEventListener('input', () => { updateClearBtn(); renderFn(); });
    ...
    setupGallerySearch('community-search-toggle-btn', 'community-search-input', () => { if (_communityCatalog) renderCommunityGrid(_communityCatalog); });
    let activeCommunitySort = 'default';
    setupGallerySort('community-sort-btn', () => activeCommunitySort, (v) => { activeCommunitySort = v; }, () => { if (_communityCatalog) renderCommunityGrid(_communityCatalog); });
    ...
    async function checkThemeUpdate(index) {
        ...
        try {
            ...
            if (semverCompare(remoteVer, localVer) > 0) {
                if (isUpdateIgnored(snap.updateUrl, localVer, remoteVer)) return null; // user dismissed this transition
    ...
    async function checkUpdatesViaManifest(snapshots) {
        ...
        for (let i = 0; i < snapshots.length; i++) {
            ...
            if (semverCompare(remoteVer, localVer) > 0) {
                if (isUpdateIgnored(snap.updateUrl, localVer, remoteVer)) {
                    results.push(null); // user dismissed this transition
                } else {
    ...
    function renderCommunityGrid(catalog) {
        ...
        // Tag original indices before filtering/sorting
        themes = themes.map((t, i) => ({ ...t, _origIdx: i }));
        ...
        if (searchQuery) {
            themes = themes.filter(t => {
                ...
                return name.includes(searchQuery) || desc.includes(searchQuery) || author.includes(searchQuery);
            });
        }

        // Sort
        if (activeCommunitySort === 'asc') {
            themes = [...themes].sort((a, b) => (a.name || '').localeCompare(b.name || ''));
        } else if (activeCommunitySort === 'desc') {
            themes = [...themes].sort((a, b) => (b.name || '').localeCompare(a.name || ''));
        }
        ...
        const cards = themes.map((t, idx) => {
            ...
            const descEscaped = (t.description || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

            return `<div class="preset-btn community-card" data-community-idx="${t._origIdx}" ${descEscaped ? `data-tooltip="${descEscaped}"` : ''}>
                ${statusBadge}
                ...
                ${badgesHTML}
                <button class="community-install-btn ${btnClass}" onclick="event.stopPropagation(); installCommunityTheme(${t._origIdx})">${btnLabel}</button>
            </div>`;
        });
    ...
    $('check-updates-btn').addEventListener('click', async () => {
        ...
        if (updates.length > 0) {
            ...
            updates.forEach((u, idx) => {
                ...
                html += `<div id="${diffId}" style="display:none; padding: 0 12px 12px; border-top: 1px solid var(--border);"></div>`;
                html += `</div>`;
            });
    ...
    // --- Ignored Updates ---
    let ignoredUpdates = Storage.get('ignored_updates', {});
    // Storage.get returns [] by default for arrays, but we pass {} for object
    if (Array.isArray(ignoredUpdates)) ignoredUpdates = {};

    function isUpdateIgnored(updateUrl, localVersion, remoteVersion) {
        if (!updateUrl) return false;
        const entry = ignoredUpdates[updateUrl];
        if (!entry) return false;
        // Legacy format (plain string): matched the remote version regardless
        // of the local version. Migrate lazily to the transition pair so a
        // later local-version change makes the update visible again.
        if (typeof entry === 'string') {
            if (entry !== remoteVersion) return false;
            ignoreUpdate(updateUrl, localVersion, remoteVersion);
            return true;
        }
        return entry.from === localVersion && entry.to === remoteVersion;
    }

    function ignoreUpdate(updateUrl, localVersion, remoteVersion) {
        if (!updateUrl) return;
        // Ignore the specific transition (from → to), not the target version:
        // if the user's local version changes (e.g. manually edited in the
        // library), the same remote version is offered again.
        ignoredUpdates[updateUrl] = { from: localVersion || '0.0.0', to: remoteVersion };
        Storage.set('ignored_updates', ignoredUpdates);
    }

    // --- Update Diff Builder ---
    const MODE_DOTS = { dark: '#6366f1', light: '#fbbf24', oled: '#1e1e1e', her: '#ec4899' };
    let _updateDiffCounter = 0;

    function buildUpdateDiffHtml(local, remote) {
        const prefix = `ud${_updateDiffCounter++}`;
        _codeDiffIdCounter = 0;
        let html = '<div class="update-diff-container">';

        MODES.forEach(mode => {
            const lm = local[mode];
            const rm = remote[mode];
            if (!lm || !rm) return;

            const changes = [];

            // Palette
            const paletteChanged = lm.h !== rm.h || lm.c !== rm.c || lm.l !== rm.l;
            if (paletteChanged) {
                changes.push({ name: 'Palette', html: _renderPaletteDiff({
                    src: { h: lm.h, c: lm.c, l: lm.l }, tgt: { h: rm.h, c: rm.c, l: rm.l },
                    srcOverrides: lm.overrides || {}, tgtOverrides: rm.overrides || {}
                }) });
            }

            // Overrides
            const lOvr = lm.overrides || {}, rOvr = rm.overrides || {};
            const ovrKeys = [...new Set([...Object.keys(lOvr), ...Object.keys(rOvr)])];
            const ovrDiffs = ovrKeys.filter(k => lOvr[k] !== rOvr[k]);
            if (ovrDiffs.length > 0) {
                changes.push({ name: `Overrides (${ovrDiffs.length})`, html: _renderOverridesDiff({ src: lOvr, tgt: rOvr }) });
            }

            // Custom CSS
            const lCss = (lm.customCSS || '').trim(), rCss = (rm.customCSS || '').trim();
            if (lCss !== rCss) {
                changes.push({ name: 'Custom CSS', html: _renderCodeDiffClickable(lCss, rCss, mode, `${prefix}-css`, '', '') });
            }

            // Canvas FX
            const lCanvas = (lm.canvasScript || '').trim(), rCanvas = (rm.canvasScript || '').trim();
            if (lCanvas !== rCanvas) {
                changes.push({ name: 'Canvas FX', html: _renderCodeDiffClickable(lCanvas, rCanvas, mode, `${prefix}-canvas`, '', '') });
            }

            // Gradient
            const gradientSnapshot = (d) => ({ stops: d.gradientStops || [], type: d.gradientType || 'linear', angle: d.gradientAngle ?? 135, intensity: d.gradientIntensity ?? 85, anim: !!d.gradientAnimation, radialPosX: d.gradientRadialPosX ?? 50, radialPosY: d.gradientRadialPosY ?? 50, radialShape: d.gradientRadialShape || 'ellipse', radialSize: d.gradientRadialSize || 'farthest-corner', meshPoints: d.gradientMeshPoints || [], meshBgColor: d.gradientMeshBgColor || '#0a0a12' });
            const lSnap = gradientSnapshot(lm), rSnap = gradientSnapshot(rm);
            const hasGradientData = lSnap.stops.length > 0 || rSnap.stops.length > 0 || lSnap.meshPoints.length > 0 || rSnap.meshPoints.length > 0;
            if (hasGradientData && JSON.stringify(lSnap) !== JSON.stringify(rSnap)) {
                changes.push({ name: 'Gradient', html: _renderGradientDiff({ src: lSnap, tgt: rSnap }) });
            }

            const modeLabel = mode.charAt(0).toUpperCase() + mode.slice(1);
            const dotColor = MODE_DOTS[mode] || '#888';
            const dotBorder = mode === 'oled' ? '; border: 1px solid #444' : '';
            const sectionId = `${prefix}-${mode}`;
            const badge = changes.length > 0
                ? `<span class="update-diff-badge changed">${changes.length} change${changes.length !== 1 ? 's' : ''}</span>`
                : `<span class="update-diff-badge same">✓ in sync</span>`;

            html += `<div class="update-diff-section">`;
            html += `<div class="update-diff-header" onclick="window._toggleUpdateDiffMode('${sectionId}', this)">`;
            html += `<span class="mode-dot" style="background:${dotColor}${dotBorder}"></span>`;
            html += `${modeLabel} Mode ${badge}`;
            html += `<span class="chevron">▼</span></div>`;
            html += `<div class="update-diff-body" id="${sectionId}">`;

            if (changes.length > 0) {
                changes.forEach(ch => {
                    html += `<div class="update-diff-sub">${ch.name}</div>`;
                    html += ch.html;
                });
            } else {
                html += `<div style="font-size:0.55rem; color:var(--text-muted); opacity:0.6; padding:4px 0;">No changes in this mode.</div>`;
            }
    ...
    window._toggleUpdateDiffMode = function(id, headerEl) {
    ...
    window._toggleBatchDiff = async function(diffId, btn, snapIndex, viaManifest) {
        ...
        let remote;
        if (viaManifest) {
            // Lazy-fetch remote data for manifest-based updates
            btn.textContent = '⏳';
            btn.disabled = true;
            try {
                const url = window._pendingBatchMeta[snapIndex];
                remote = await fetchThemeData(url);
            } catch (e) {
                panel.innerHTML = `<div style="font-size:0.65rem; color:#ef4444; padding:8px 0;">Failed to load diff: ${_esc(e.message)}</div>`;
                panel.style.display = 'block';
                btn.textContent = '▾ Diff';
                btn.disabled = false;
                return;
            ...
            remote = window._pendingBatchUpdates[snapIndex];
        }

        if (!remote) {
            panel.innerHTML = `<div style="font-size:0.65rem; color:var(--text-muted); opacity:0.6; padding:8px 0;">No diff data available.</div>`;
        } else {
            panel.innerHTML = buildUpdateDiffHtml(local, remote);
        }
    ...
    window.checkSingleUpdate = async (index) => {
        ...
        const diffPanel = $('update-diff-panel');
        const diffToggleBtn = $('update-diff-toggle-btn');
        if (result.remote) {
            const diffHtml = buildUpdateDiffHtml(result.local, result.remote);
            diffPanel.innerHTML = diffHtml;
            diffPanel.style.display = 'none';
            diffToggleWrap.style.display = '';
            diffToggleBtn.textContent = '▸ View Changes';
        } else {
    ...
    $('update-ignore-btn').addEventListener('click', () => {
        if (pendingUpdate) {
            ignoreUpdate(pendingUpdate.local.updateUrl, pendingUpdate.localVersion, pendingUpdate.remoteVersion);
            showToast(`Ignored v${pendingUpdate.localVersion} → v${pendingUpdate.remoteVersion} for "${pendingUpdate.local.name}"`);
        }
        hideModal('update-modal');
        pendingUpdate = null;
    });

    $('update-diff-toggle-btn').addEventListener('click', () => {
        const panel = $('update-diff-panel');
        const btn = $('update-diff-toggle-btn');
        const isOpen = panel.style.display !== 'none';
    ...
    (function enforceValves() {
        ...
        function setBadgeState(state) {
            ...
            badge.classList.remove('active', 'draft');
            if (state === 'active') {
                badge.classList.add('active');
                badge.setAttribute('data-tooltip', 'Theme is active \u2014 changes are pushed to all users in real-time');
                badgeText.textContent = 'Theme Active';
            } else if (state === 'draft') {
                badge.classList.add('draft');
                badge.setAttribute('data-tooltip', 'Draft mode \u2014 changes stay local until you publish');
                badgeText.textContent = 'Draft Active';
            } else {
                badge.setAttribute('data-tooltip', 'Function is disabled \u2014 toggle it ON in Admin \u2192 Functions');
        ...
        // Deferred until after page load to avoid "interrupted while loading" errors.
        ...
        if (document.readyState === 'complete') {
            ...
            window.addEventListener('load', () => setTimeout(initBadgeSSE, 100));
        }
        ...
        if (!cfg.enableCommunityThemes) {
            ...
            // Expand theme gallery to fill the page since community section is gone
        ...
        // Shared error banner for both restriction branches below
        const showImportBlocked = (opts, msg) => {
        ...
        if (!cfg.enableUrlImport) {
            ...
            loadFromUrl = function(opts) {
                showImportBlocked(opts, '⛔ URL imports are disabled by your administrator.');
            };
        ...
        if (cfg.autoSync) {
            const syncAllWrap = document.getElementById('sync-all-toggle-wrap');
            if (syncAllWrap) syncAllWrap.style.display = '';
            const syncBtn = document.getElementById('sync-mode-btn');
            if (syncBtn) syncBtn.setAttribute('data-tooltip', 'Instantly sync all settings from the current mode to every other mode');
        }

        // ── 5. Auth page theming — force off + hide toggles + overwrite localStorage ──
    ...
    (function applyAITheme() {
        ...
        toast.innerHTML = `🤖 <strong>${_esc(desc)}</strong>${modeLabel} — ${features} — Fine-tune below`;
    ...
    function renderSnapshots() {
        ...
        try {
            ...
            container.innerHTML = filteredSnapshots.map((s) => {
                ...
                // Use the global _esc() for HTML escaping (more robust than manual replace)
                let ttHtml = `<div class="tt-row"><span class="tt-name">${_esc(s.name)}</span>`;
                if (s.version) ttHtml += `<span class="tt-version">v${_esc(s.version)}</span>`;
                ttHtml += `</div>`;
                if (s.author) ttHtml += `<div class="tt-author">by ${_esc(s.author)}</div>`;
                if (s.description) ttHtml += `<div class="tt-desc">${_esc(s.description)}</div>`;
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        valve_config = {
            ...
            "autoSync": self.valves.auto_sync,
        ...
        # --- Generate templated HTML fragments ---
        def _metadata_grid(prefix, fields):
            """Generate a metadata-grid with the given ID prefix."""
            html = '<div class="metadata-grid">\n'
            for fid, label, placeholder, full_width in fields:
                style = ' style="grid-column: 1 / -1;"' if full_width else ''
                html += f'                <div class="metadata-field"{style}>\n'
                html += f'                    <label>{label}</label>\n'
                html += f'                    <input type="text" id="{prefix}-{fid}" placeholder="{placeholder}">\n'
                html += '                </div>\n'
            html += '            </div>'
            return html

        # Shared field definitions (fid_suffix, label, placeholder, full_width)
        _shared_meta = [
            ('desc-input', 'Description', 'A short description', False),
            ('author-input', 'Author', 'Your name', False),
            ('version-input', 'Theme Version', '1.0.0', False),
            ('target-input', 'Target WebUI Version', '0.9.0', False),
            ('repo-input', 'Repository URL', 'https://github.com/...', False),
            ('update-input', 'Theme Update URL', 'https://...', True),
        ]
        save_meta = _metadata_grid('theme', [
            ('name-input', 'Name', 'My Custom Theme', False),
        ] + _shared_meta)
        rename_meta = _metadata_grid('rename', [
            ('input', 'Name', 'Theme Name', False),
        ] + _shared_meta)

        # Sync option blocks
        _sync_opts = [
            ('palette', 'OKLCH Palette', 'L, C, H base color values'),
            ('overrides', 'Variable Overrides', 'Manual 50-950 ramp steps &amp; locks'),
            ('css', 'Custom CSS Snippet', 'All raw styles from Custom CSS tab'),
            ('canvas', 'Canvas FX Script', 'Background animation code &amp; state'),
            ('gradient', 'Gradient Background', 'Color stops, direction, type &amp; animation'),
            ('auth', 'Auth Page Visibility', "'Show on Auth Pages' toggles for all tabs"),
        ]
        sync_html = '\n\n'.join(
            f'                <div class="sync-option">\n'
            f'                    <input type="checkbox" id="sync-opt-{key}" checked>\n'
            f'                    <label>\n'
            f'                        {label} <span class="sync-delta-badge" id="sync-badge-{key}" style="display:none;"></span>\n'
            f'                        <span>{desc}</span>\n'
            f'                    </label>\n'
            f'                    <span class="sync-diff-chevron" data-setting="{key}" style="display:none;">▾</span>\n'
            f'                </div>\n'
            f'                <div class="sync-diff-panel" id="sync-diff-{key}" style="display:none;"></div>'
            for key, label, desc in _sync_opts
        )

        # JSON editor modals
        def _json_modal(modal_id, lang_label, textarea_id, readonly, badge_id,
                        collapse_id, close_id, close_tooltip, buttons=None):
            ro = ' readonly' if readonly else ''
            html = (
                f'    <div id="{modal_id}" class="modal-overlay z-high" style="padding: 24px;">\n'
                f'        <div style="width: 100%; max-width: 600px; max-height: 90%; display: flex; flex-direction: column; gap: 0; animation: containerShow 0.3s cubic-bezier(0.16, 1, 0.3, 1);">\n'
                f'            <div class="owui-code-block" style="flex: 1; display: flex; flex-direction: column; min-height: 0; max-height: 100%;">\n'
                f'                <div class="owui-code-header">\n'
                f'                    <span class="owui-code-lang">{lang_label}</span>\n'
                f'                    <div class="owui-code-actions">\n'
                f'                        <button class="owui-code-btn" id="{collapse_id}" data-tooltip="Collapse JSON">Collapse</button>\n'
                f'                        <button class="owui-code-btn copy-css-btn" data-target="{textarea_id}" data-tooltip="Copy JSON to Clipboard">Copy</button>\n'
                f'                        <button class="owui-code-btn" id="{close_id}" style="color: var(--text-muted);" data-tooltip="{close_tooltip}">Close</button>\n'
                f'                    </div>\n'
                f'                </div>\n'
                f'                <textarea id="{textarea_id}"{ro} spellcheck="false" style="flex: 1; min-height: 400px; cursor: text; user-select: text;"></textarea>\n'
                f'            </div>\n'
            )
            if buttons:
                html += (
                    f'            <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 0 0; gap: 8px;">\n'
                    f'                <span id="{badge_id}" style="font-size: 0.65rem; font-weight: 700; color: #22c55e; opacity: 0.7;">Valid JSON</span>\n'
                    f'                <div style="display: flex; gap: 8px;">\n'
                )
                for btn_id, btn_label, btn_cls in buttons:
                    html += f'                    <button class="{btn_cls}" id="{btn_id}">{btn_label}</button>\n'
                html += '                </div>\n            </div>\n'
            else:
                html += (
                    f'            <div style="display: flex; justify-content: flex-end; padding: 10px 0 0; gap: 8px;">\n'
                    f'                <span id="{badge_id}" style="font-size: 0.65rem; font-weight: 700; color: #22c55e; opacity: 0.7;">Valid JSON</span>\n'
                    f'            </div>\n'
                )
            html += '        </div>\n    </div>'
        ...
        json_modals = '\n\n'.join([
            _json_modal('json-view-modal', 'json', 'json-view-textarea', True,
                        'json-valid-badge', 'json-collapse-btn', 'json-close-btn', 'Close JSON Viewer'),
            _json_modal('snapshot-json-edit-modal', 'json \u2014 Manual Snapshot Editor', 'snapshot-json-edit-textarea', False,
                        'snapshot-json-valid-badge', 'snapshot-json-collapse-btn', 'snapshot-json-cancel-btn', 'Close without saving',
                        [('snapshot-json-discard-btn', 'Cancel', 'btn'), ('snapshot-json-save-btn', 'Save Changes', 'btn btn-primary')]),
            _json_modal('save-json-edit-modal', 'json \u2014 New Theme Editor', 'save-json-edit-textarea', False,
                        'save-json-valid-badge', 'save-json-collapse-btn', 'save-json-cancel-btn', 'Close without saving',
                        [('save-json-discard-btn', 'Cancel', 'btn'), ('save-json-save-btn', 'Save as New Theme', 'btn btn-primary')]),
        ])
        ...
        final_html = (
            ...
            .replace("{JSON_EDITOR_MODALS}", json_modals)
    ...
    # Per-worker ceiling on concurrent SSE connections. The endpoint is
    ...
    @classmethod
    def _apply_sidebar_transparency(cls, css: str, mode: str) -> str:
        """Post-process flat CSS for the sidebar transparency valve.

        Composition of the per-rule rewrite (safe to run on individual
        sections) and the appended mobile-upgrade block (must be added
        exactly once, at the end of the delivered CSS).
        """
        css = cls._rewrite_sidebar_fx(css, mode)
        if mode == "transparent":
            css += cls._sidebar_mobile_css()
        return css

    @staticmethod
    def _rewrite_sidebar_fx(css: str, mode: str) -> str:
        """Rewrite opaque /*[FX]*/ sidebar rules for the transparency valve.
        ...
        Plain-mode sidebar rules (no marker) are left untouched. Operates on
        individual rules, so it can be applied to the flat CSS or to the
        structural/gradient sections independently.
        """
        if mode == 'translucent':
    ...
    @staticmethod
    def _sidebar_mobile_css() -> str:
        """Mobile override: auto-upgrade transparent sidebar → translucent.

        Uses #sidebar#sidebar for specificity to override the transparent rules.
        Appended once at the end of the delivered CSS (the 'extra' section when
        assembling from structured sections).
        """
        return (
    ...
    @classmethod
    def _apply_overlay_transparency(cls, css: str) -> str:
        """Append translucent glass CSS for portaled overlay UI elements."""
        return css + cls._overlay_css()

    @staticmethod
    def _overlay_css() -> str:
        """Return the translucent glass CSS block for portaled overlay UI.

        Targets settings modal, dropdown menus, user menus, integration menus,
        and other UI elements portaled outside .app directly on <body>.
        Uses descendant selectors to handle varying portal DOM depths.
        Appended at the end of the delivered CSS (the 'extra' section when
        assembling from structured sections).
        """
        blur = 'blur(20px) saturate(1.3)'

        # Selector fragment: portal wrappers (direct children of body that aren't part of the app/theme system)
        pex = ':not(.app):not(#theme-designer-container):not([id^="owui-"]):not(#svelte):not(script):not(style)'
        ...
        def _decl(color_var):
            return (
                ...
                f'  backdrop-filter: {blur} !important;\n'
                f'  -webkit-backdrop-filter: {blur} !important;\n'
            )
        ...
        css = '\n/* --- Overlay Translucency (valve-injected) --- */\n'
        for sels, color_var in rules:
            css += ',\n'.join(sels) + ' {\n' + _decl(color_var) + '}\n'
    ...
    def _postprocess_css(self, css):
        """Apply valve-based CSS post-processing (sidebar + overlay transparency)."""
        if self.valves.sidebar_transparency != "opaque":
            css = self._apply_sidebar_transparency(css, self.valves.sidebar_transparency)
        if self.valves.overlay_transparency != "opaque":
            css = self._apply_overlay_transparency(css)
        return css

    def _postprocess_sections(self, sections: dict) -> dict:
        """Per-section mirror of _postprocess_css.

        The sidebar valve rewrites /*[FX]*/ rules, which live only in the
        structural and gradient sections. Valve-APPENDED rules (mobile sidebar
        upgrade, overlay glass) go into a synthetic 'extra' section so
        consumers assembling from sections deliver the same CSS as /theme.css.
        'extra' is never auth-gated by the bootloader.
        """
        out = dict(sections)
        extra = ""
        sidebar_mode = self.valves.sidebar_transparency
        if sidebar_mode != "opaque":
            out["structural"] = self._rewrite_sidebar_fx(out["structural"], sidebar_mode)
            out["gradient"] = self._rewrite_sidebar_fx(out["gradient"], sidebar_mode)
            if sidebar_mode == "transparent":
                extra += self._sidebar_mobile_css()
        if self.valves.overlay_transparency != "opaque":
            extra += self._overlay_css()
        out["extra"] = extra
        return out

    def _state_with_sections(self, state: str) -> str:
        """Merge valve-postprocessed CSS sections into a state JSON string.

        Injected at delivery time under the reserved '_cssSections' key so the
        bootloader can assemble auth-page CSS from structured sections instead
        of regex-slicing the flat blob. No-op when the sections file is missing
        (legacy flat-CSS saves — bootloader falls back to marker regexes) or
        the state isn't parseable JSON (served as-is by design).
        """
        sections = self._load_sections()
        if not sections:
            return state
        try:
            data = _json.loads(state)
            if not isinstance(data, dict):
                return state
        except Exception:
            return state
        data["_cssSections"] = self._postprocess_sections(sections)
        return _json.dumps(data, separators=(",", ":"))

    def _postprocess_state(self, state):
        """Apply valve-based state post-processing (strip canvas if disabled)."""
        if state and not self.valves.enable_canvas_fx:
    ...
    def _redis_publish(cls, msg: str):
        ...
        The blocking publish is dispatched to the default thread-pool executor
        ...
        try:
            ...
            loop = None
        if loop is not None:
            loop.run_in_executor(None, cls._redis_publish_sync, msg)
        else:
            cls._redis_publish_sync(msg)
    ...
    def _redis_publish_sync(cls, msg: str):
        ...
        try:
            ...
            try:
                ...
                pass
            # Bounded timeouts: even off-loop, a dead route shouldn't pin a
            # thread-pool worker for minutes waiting on TCP defaults.
    ...
    def _broadcast_update(cls, etags: dict | None = None):
        ...
        /state.json, which apply valve post-processing at delivery time. This
        keeps SSE frames tiny instead of shipping the full CSS + state
        (including the entire canvas script) to every client on every
        ...
        data = {"v": int(time.time() * 1000)}
        if etags:
            if etags.get("cssTag"):
                data["cssTag"] = etags["cssTag"]
            if etags.get("stateTag"):
