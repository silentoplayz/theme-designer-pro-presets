...
**Closed the remaining injection paths.** 1.7.4 sanitized the generated stylesheet; several surfaces in the admin's own browser were still rendering remote values raw. Theme names and versions went into inline `onclick` handlers in the update list, where a crafted name executed as JavaScript — those values now travel through escaped `data-` attributes read by a delegated handler, so they are never in executable position at all. The tonal-ramp preview, the gradient preset swatches and stop rows, and the mesh dots all sanitize now.

The theme library's rich tooltip was the worst of them: its markup was escaped into an attribute and then read back out with `getAttribute` and assigned to `innerHTML`. The HTML parser decodes entities when it builds an attribute value, so the escaping was undone before the markup reached the sink and a theme named `<img src=x onerror=...>` executed on hover. Rich tooltips are no longer round-tripped through the DOM at all.

**Updates that install code now ask, on every path.** 1.7.5 added a warning for updates that add or change a Canvas FX script. It compared script bodies, but catalog updates do not carry bodies — so it always concluded "no script change" and stayed silent on exactly the path where a hostile script would arrive. Catalog updates now report the risk as *unknown* rather than *none*.

More importantly, the confirmation itself now lives in the one function every update path funnels through. It used to sit on the catalog path alone, which meant the paths that had *already downloaded* the payload — "Update All" and the per-row Update button, where an added script is a known fact rather than a possibility — installed it with no prompt at all. An update that flips an existing script from disabled to enabled warns too, as does one that changes instance-wide custom CSS.

**Allowed Import Domains no longer has a bypass.** The check existed twice, and the older copy failed *open* on a URL it could not parse and matched hosts case-sensitively, so `EVIL.COM` passed an allowlist written in lowercase. There is now one check, and one URL rewriter feeding it: they used to be separate, and the manual import path had a `gist.github.com` rewrite the check did not know about, so a gist was approved against one host and fetched from another. Every remote fetch now re-validates after redirects, so an allowed host cannot hand off to a blocked one, and the community catalog, manifest and bundle go through the same gate — a restrictive allowlist stops the automatic update traffic too, not just the manual button. Every remote response is size-capped, which it was not on any of the automated paths.

**Gradient presets are validated on import.** They are a separate import format with their own file type, and nothing ever validated them — so a preset carrying a crafted colour reached the designer's own markup unchecked. They now go through the same normalization as everything else, at the one function through which every preset becomes live theme data. Themes restored from the server library and from older local storage are validated at the door as well.

**Also fixed:** conic gradients fell through to a linear gradient in one CSS builder and still previewed as linear in two others; the animation-speed clamp enforced its floor but not its ceiling; "Update All" reported success for updates that failed, and now reports what actually happened with a Retry Failed action that retries only the failures; a non-string value in remote theme JSON silently killed the update dialog; a malformed colour could abort CSS generation for a whole theme.
