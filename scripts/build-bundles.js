...
function buildCanvasFxBundle() {
  ...
  const presets = files.map(f => {
    ...
    const name = titleMatch
      ? titleMatch[1].trim()
      : f.replace('.js', '').split('_').filter(Boolean).map(w => w[0].toUpperCase() + w.slice(1)).join(' ');
    return { name, script };
  });
...
function buildCssBundle() {
  ...
  const presets = files.map(f => {
    ...
    const name = f.replace('.css', '').split('_').filter(Boolean).map(w => w[0].toUpperCase() + w.slice(1)).join(' ');
    return { name, code };
  });
...
function buildGradientBundle() {
  ...
  // Combined bundle
  const allBundle = {
    gradient_presets: all,
    isGradientBackup: true,
    version: '1.0.0',
    _meta: { count: all.length },
  };
  const outPath = path.join(BUNDLES_DIR, 'gradients-all.json');
  fs.writeFileSync(outPath, JSON.stringify(allBundle, null, 2));
  ...
  for (const group of ['still', 'animated']) {
    ...
    if (groupPresets.length > 0) {
      const groupBundle = {
        gradient_presets: groupPresets,
        isGradientBackup: true,
        version: '1.0.0',
        _meta: { count: groupPresets.length },
      };
      const gPath = path.join(BUNDLES_DIR, `gradients-${group}.json`);
      fs.writeFileSync(gPath, JSON.stringify(groupBundle, null, 2));
...
function buildEverythingBundle() {
  const bundle = {
    _meta: { description: 'Combined bundle — all preset types' },
    version: '1.0.0',
  };
  let parts = [];
  ...
  if (fs.existsSync(fxDir)) {
    ...
    if (files.length > 0) {
      bundle.canvas_presets = files.map(f => {
        const script = fs.readFileSync(path.join(fxDir, f), 'utf8');
        const titleMatch = script.match(/\*\s*Title:\s*(.+)/);
        const name = titleMatch
          ? titleMatch[1].trim()
          : f.replace('.js', '').split('_').filter(Boolean).map(w => w[0].toUpperCase() + w.slice(1)).join(' ');
  ...
  if (fs.existsSync(cssDir)) {
    ...
    if (files.length > 0) {
      bundle.css_presets = files.map(f => ({
        name: f.replace('.css', '').split('_').filter(Boolean).map(w => w[0].toUpperCase() + w.slice(1)).join(' '),
